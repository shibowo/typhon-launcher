...
// checks if configured path exists in any logical way (absolute path,in home or
// in themedatadir)
std::string checkpath(std::string orgpath, std::string defaultpath) {
  std::string wipcheck;
  if (strcmp(orgpath.c_str(), "") != 0) wipcheck = orgpath;
  FILE *pathcheck = fopen(wipcheck.c_str(), "r");

  if (pathcheck) {
    fclose(pathcheck);
  } else {
    wipcheck = setEnvhome(wipcheck);
  }
  ...
  if (pathcheck) {
    ...
    wipcheck = rtcfg->sub("scanned")->getString("themedata") + "/" +
               tycfg->getString("themename") + "/" + orgpath;
  }
  pathcheck = fopen(wipcheck.c_str(), "r");
  if (pathcheck) {
    fclose(pathcheck);
...
void checkpaths() {
  ...
  for (int p = 0; p < typa->size(); p++) {
    ...
    rtcfg->sub("scanned")->set(
        path, checkpath(typa->getString(path), rtcfg->getString("datadir")), p);
    q = p + 1;
  }
  ...
  for (int p = 0; p < thpa->size(); p++) {
    ...
    rtcfg->sub("scanned")->set(
        path, checkpath(thpa->getString(path), rtcfg->getString("datadir")),
        p + q);
  }
}
...
void cfgpath() {
  ...
  rtcfg->set("rominfodir", rtcfg->getString("cfgdir") + "/rominfo/");
  mkdir(rtcfg->getString("rominfodir").c_str(), 0755);
...
void checkcfg() {
  ...
  cfgsubdir = rtcfg->getString("cfgdir") + "/previews/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir += "/programpictures/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/videos/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/music/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/p3ts/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/shaders/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/previews/videopictures/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/previews/videopreviews/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/previews/icons/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/export/";
  mkdir(cfgsubdir.c_str(), 0755);
#ifdef WITHMMD
  cfgsubdir = rtcfg->getString("cfgdir") + "/models/";
  mkdir(cfgsubdir.c_str(), 0755);
  cfgsubdir = rtcfg->getString("cfgdir") + "/motion/";
  mkdir(cfgsubdir.c_str(), 0755);
#endif  // WITHMMD
}
...
// reads main config
void readtyphonxml() {
  FILE *globalxmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("globalxml").c_str(), "r");
...
void readlanguagexml() {
  std::string langxmlconfig =
      rtcfg->getString("cfgdir") + rtcfg->sub("cfgs")->getString("lang") +
      tycfg->sub("strings")->getString("language") + ".xml";
...
void readmenuxml() {
  FILE *menuxmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("menuxml").c_str(), "r");
  if (menuxmlcfgcheck) {
    loadmenuxmlcfg();
...
void readinputxml() {
  FILE *inputxmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("inputxml").c_str(), "r");
  if (inputxmlcfgcheck) {
    loadinputxmlcfg();
