...
void setrtdefaults() {
  std::string defrtconfig =
      rtcfg->getString("systemconfigdir") + "typhon-defaultruntime.xml";
...
int setlanguagedefaults() {
  std::string deflangxmlconfig = rtcfg->getString("systemconfigdir") +
                                 rtcfg->sub("cfgs")->getString("lang") +
                                 tycfg->sub("strings")->getString("language") +
                                 ".xml";
  ...
  if (xmlcfgcheck) {
    ...
    std::string enlangxmlconfig = rtcfg->getString("systemconfigdir") +
                                  rtcfg->sub("cfgs")->getString("lang") +
                                  "en.xml";
...
int setmenudefaults() {
  std::string defmenuxmlconfig = rtcfg->getString("systemconfigdir") +
                                 rtcfg->sub("cfgs")->getString("menu");
...
int setglobaldefaults() {
  std::string defglobalxmlconfig = rtcfg->getString("systemconfigdir") +
                                   rtcfg->sub("cfgs")->getString("global");
...
int setinputdefaults() {
  std::string definputxmlconfig = rtcfg->getString("systemconfigdir") +
                                  rtcfg->sub("cfgs")->getString("input");
...
int setemudefaults() {
  std::string defemuxmlconfig = rtcfg->getString("systemconfigdir") +
                                rtcfg->sub("cfgs")->getString("emu");
...
int setshaderdefaults() {
  std::string defshaderxmlconfig = rtcfg->getString("systemconfigdir") +
                                   rtcfg->sub("cfgs")->getString("shader");
...
int setthemedefaults() {
  std::string defthemexmlconfig =
      rtcfg->getString("systemconfigdir") + "typhon.xml";
...
// save/load
void loadmenuxmlcfg() {
  mecfg->loadXML(rtcfg->sub("configs")->getString("menuxml"));
}
void savemenuxmlcfg() {
...
void loadglobalxmlcfg() {
  tycfg->loadXML(rtcfg->sub("configs")->getString("globalxml"));
}
void saveglobalxmlcfg() {
  tycfg->saveXML(rtcfg->sub("configs")->getString("globalxml"));
}
void loadinputxmlcfg() {
  incfg->loadXML(rtcfg->sub("configs")->getString("inputxml"));
}
void saveinputxmlcfg() {
  incfg->saveXML(rtcfg->sub("configs")->getString("inputxml"));
}
void loademuxmlcfg() {
  emcfg->loadXML(rtcfg->sub("configs")->getString("emuxml"));
}
void saveemuxmlcfg() {
  emcfg->saveXML(rtcfg->sub("configs")->getString("emuxml"));
}
void loadromxmlcfg() {
  rmcfg->loadXML(rtcfg->sub("configs")->getString("romxml"));
}
void loadthemexmlcfg() {
  thcfg->loadXML(rtcfg->sub("configs")->getString("themexml"));
}
void savethemexmlcfg() {
  thcfg->saveXML(rtcfg->sub("configs")->getString("themexml"));
}
...
int Ishowmenu() { return tycfg->getInt("showmenu"); }
...
bool Bvidmenu() {
  if (thcfg->sub("objects")->sub("video")->getInt("videomenu") == Ishowmenu())
    return 1;
  else
    return 0;
}
...
bool Bp3tmenu() {
  if (tycfg->sub("globalints")->getInt("p3tmenu") == Ishowmenu() &&
      p3cfg->sub("ps3themes")->size() > 0)
    return 1;
  else
    return 0;
}
...
bool Bthememenu() {
  if (tycfg->sub("globalints")->getInt("thememenu") == Ishowmenu() &&
      rtcfg->sub("themes")->size() > 0)
    return 1;
  else
    return 0;
}
...
int Ime() {
  if (Ishowmenu() == 0)
    ...
    if (rmcfg->sub("Roms")->sub(IeID() - 1) == 0)
      return 0;
    else
      return rmcfg->sub("Roms")->sub(IeID() - 1)->size();
  } else if (Bp3tmenu())
    return p3cfg->sub("ps3themes")->size();
  else if (Bthememenu())
    return rtcfg->sub("themes")->size();
  else if (Bvidmenu())
    return vicfg->sub("videos")->size();
  else
    return mecfg->sub(Ishowmenu() - 1)->sub("entry")->size();
}

void setcs(int thiscs) {
  if (Ics() > Ime())
    rtcfg->sub("curSel")->set(Ishowmenu(), Ime() - 1);
  else
    rtcfg->sub("curSel")->set(Ishowmenu(), thiscs);
}

void setem(int thisem) { mecfg->sub(Ishowmenu() - 1)->set("emumenu", thisem); }

void incem(int bythis) {
  ((TConfig<int> *)mecfg->sub(Ishowmenu() - 1)->get("emumenu"))
      ->operator+(bythis);
}

int IeID() {
  if (Iem() != 0)
    return Ishowmenu() + ((Iem() - 1) * 7);
  else
    return 0;
}
...
std::string emuargs(int thisrom) {
  if (rmcfg->sub("Roms")->sub(IeID() - 1)->sub(Ics())->exists("args"))
    return rmcfg->sub("Roms")->sub(IeID() - 1)->sub(Ics())->getString("args");
  else
    return emcfg->sub(thisrom)->getString("emuargs");
}
...
std::string mentry(int menu, int count) {
  if (menu == -1) {
    if (thcfg->sub("switches")->getBool("useemutitles"))
      return emutitle(count);
    else
      return emulator(count);
  } else {
    if (Iem() != 0)
      return getPrefix(
          rmcfg->sub("Roms")->sub(IeID() - 1)->sub(count)->getString("file"));
    else if (Bvidmenu())
      return getPrefix(vicfg->sub("videos")->sub(count)->getString("file"));
    else if (Bp3tmenu())
      return getPrefix(p3cfg->sub("ps3themes")->getString(count));
    else if (Bthememenu())
      return getPrefix(rtcfg->sub("themes")->getString(count));
    else {
      if (!mecfg->sub(menu)) {
...
std::string title(int menu) {
  if (menu == -1)
    return lacfg->getString("emulisttitle");
  else if (Bvidmenu())
    return lacfg->getString("videolisttitle");
  else if (Bp3tmenu())
    return lacfg->getString("p3tlisttitle");
  else if (Bthememenu())
    return lacfg->getString("themelisttitle");
  else
    return mecfg->sub(menu)->getString("title");
}
...
bool useShader() { return thcfg->sub("shader")->getBool("useshader"); }

int useBackShader() { return thcfg->sub("shader")->getInt("usebackshader"); }
