...
int openmodel() {
  std::string modelpath = rtcfg->sub("scanned")->getString("modeldir") + "/" +
                          thcfg->sub("files")->getString("mmdmodelfile");
  ...
  if (!mmdpath) {
    ...
    if (checkmodel) {
      ...
      thcfg->sub("objects")->sub("mmd")->setBool("enabled", 0);
    }
  }
...
void openmotion() {
  std::string motionpath = rtcfg->sub("scanned")->getString("motiondir") + "/" +
                           thcfg->sub("files")->getString("mmdmotionfile");
...
void togglephysics() {
  thcfg->sub("switches")
      ->setBoolC("mmdphysics", !thcfg->sub("switches")->getBool("mmdphysics"));
...
void displaymodel() {
  ...
  glTranslatef(-rtcfg->sub("objects")->sub("mmd")->getFloat("posx"),
               rtcfg->sub("objects")->sub("mmd")->getFloat("posy"),
               -(thcfg->sub("objects")->sub("mmd")->getInt("posz")));
  rtcfg->sub("objects")->sub("mmd")->set(
      "width", -(rtcfg->sub("objects")->sub("mmd")->getFloat("scalex") *
                 rtcfg->sub("objects")->sub("mmd")->getInt("calcx")));
  rtcfg->sub("objects")->sub("mmd")->set(
      "height", (rtcfg->sub("objects")->sub("mmd")->getFloat("scaley") *
  ...
  glTranslatef(0, -rtcfg->sub("objects")->sub("mmd")->getFloat("height") / 2,
               0);
  ...
  if ((rtcfg->getInt("picked") == thcfg->sub("objects")->get("mmd")->id) ||
    ...
    glTranslatef((rtcfg->sub("objects")->sub("mmd")->getFloat("scalex") *
                  rtcfg->sub("objects")->sub("mmd")->getInt("calcx")) /
                     2,
                 0, 0);
    drawquad(-(rtcfg->sub("objects")->sub("mmd")->getFloat("scalex") *
               rtcfg->sub("objects")->sub("mmd")->getInt("calcx")),
  ...
  if (thcfg->sub("objects")->sub("mmd")->getBool("enabled")) {
    glScalef(rtcfg->sub("objects")->sub("mmd")->getFloat("scalex"),
             rtcfg->sub("objects")->sub("mmd")->getFloat("scaley"),
...
void listmmd(std::string searchdir, std::string suffix, std::string cat1,
             std::string cat2) {
  struct dirent **sdit;
  int num_entries;
  if ((num_entries =
           scandir((searchdir).c_str(), &sdit, fileselectAll, alphasort)) < 0) {
  } else {
    int k = 0;
...
void updatemodel(bool up) {
  if (up) {
    if (rtcfg->getInt("currentmodel") == rtcfg->sub("models")->size() - 1)
      rtcfg->set("currentmodel", 0);
    else {
      ((TConfig<int> *)rtcfg->get("currentmodel"))->operator+(1);
    }
  } else {
    if (rtcfg->getInt("currentmodel") == 0)
      rtcfg->set("currentmodel", rtcfg->sub("models")->size() - 1);
    else {
      ((TConfig<int> *)rtcfg->get("currentmodel"))->operator-(1);
    }
  }
  thcfg->sub("files")->setC("mmdmodelfile", rtcfg->sub("models")->getString(
                                                rtcfg->getInt("currentmodel")));
  if (thcfg->sub("objects")->sub("mmd")->getBool("enabled")) {
    openmodel();
  }
  shownote(lacfg->getString("pmd") +
           getPrefix(thcfg->sub("files")->getString("mmdmodelfile")));
}

void updatemotion(bool up) {
  if (up) {
    if (rtcfg->getInt("currentmotion") == rtcfg->sub("motion")->size() - 1)
      rtcfg->set("currentmotion", 0);
    else {
      ((TConfig<int> *)rtcfg->get("currentmotion"))->operator+(1);
    }
  } else {
    if (rtcfg->getInt("currentmotion") == 0)
      rtcfg->set("currentmotion", rtcfg->sub("motion")->size() - 1);
    else {
      ((TConfig<int> *)rtcfg->get("currentmotion"))->operator-(1);
    }
  }
  thcfg->sub("files")->setC(
      "mmdmotionfile",
      rtcfg->sub("motion")->getString(rtcfg->getInt("currentmotion")));
  if (thcfg->sub("objects")->sub("mmd")->getBool("enabled")) {
    openmotion();
  }
  shownote(lacfg->getString("vmd") +
           getPrefix(thcfg->sub("files")->getString("mmdmotionfile")));
}

void togglemotionloop() {
  thcfg->sub("switches")
      ->setBoolC("mmdmotionloop",
                 !thcfg->sub("switches")->getBool("mmdmotionloop"));
  g_clPMDModel.setLoop(thcfg->sub("switches")->getBool("mmdmotionloop"));
}
