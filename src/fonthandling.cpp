...
FXFont font, musicdurationfontfx, titlefontfx, clockfontfx, datefontfx,
    settingsfontfx, sysinfofontfx, notifyfontfx, menuselfontfx, rssfontfx,
    counterfontfx, rominfofontfx;
...
void textcol(std::string thistext, FXFont* thisfont, int alpha) {
  if (rtcfg->getInt("picked") == thcfg->sub("objects")->get(thistext)->id &&
      pick() != 3 && pick() != 4) {
    if (rtcfg->sub("block")->getBool(
            thcfg->sub("objects")->get(thistext)->id)) {
    ...
    if (pick() == 4 &&
        !thcfg->sub("objects")->sub(thistext)->getBool("enabled")) {
      ...
      if (thcfg->sub("switches")->getBool("usemonthcolors")) {
        ...
        thisfont->setColour(vec4f(mc->getFloat("R"), mc->getFloat("G"),
                                  mc->getFloat("B"), norm255(alpha)));
      } else

      {
        TyphonConfig* co = thcfg->sub("objects")->sub(thistext);
        thisfont->setColour(
            veccol4f(co->getInt("R"), co->getInt("G"), co->getInt("B"), alpha));
      }
    }
  }
}
...
void textrot(std::string thistext, std::string drawtext, FXFont* thisfont,
             int heightcheck) {
  ...
  if (thcfg->sub("switches")->getBool("rotateselection") &&
      strcmp(thistext.c_str(), "menu") == 0) {
  ...
  float ry =
      (tt->getInt("rotx"));  // vector rx,ry has rotationindependant length
...
void switchfont(bool up) {
  if (rtcfg->sub("fonts")->size() > 0) {
    if (up) {
      if (rtcfg->getInt("fontcount") == 0) {
        rtcfg->set("fontcount", rtcfg->sub("fonts")->size() - 1);
      } else {
        ((TConfig<int>*)rtcfg->get("fontcount"))->operator-(1);
      }
    } else {
      if (rtcfg->getInt("fontcount") == rtcfg->sub("fonts")->size() - 1) {
        rtcfg->set("fontcount", 0);
      } else {
        ((TConfig<int>*)rtcfg->get("fontcount"))->operator+(1);
      }
    }
    thcfg->sub("files")->setC("fontfile", rtcfg->sub("fonts")
                                              ->sub(rtcfg->getInt("fontcount"))
                                              ->getString("file"));
    updatefont();
  }
}
...
void listfonts() {
  ...
  int num_entries;
  if ((num_entries =
           scandir((rtcfg->sub("scanned")->getString("fontpath")).c_str(),
                   &sdit, fileselectAll, alphasort)) < 0) {
  } else {
    int k = 0;
    for (int j = 0; j < num_entries; j++) {
      if (strstr(sdit[j]->d_name, ".ttf") || strstr(sdit[j]->d_name, ".otf")) {
        std::stringstream entry;
        entry << "fonts" << k;
        rtcfg->sub("fonts")
            ->sub(entry.str(), k++)
            ->set("file", sdit[j]->d_name);
      }
    }
  }
}
...
void drawmenuseltext(std::string thistext) {
  ...
  if (rtcfg->getBool("iscursel")) {
    menuselfontfx.setColour(
        veccol4f(rtcfg->getInt("tempselr"), rtcfg->getInt("tempselg"),
                 rtcfg->getInt("tempselb"), rtcfg->getInt("menualpha")));
  } else {
    menuselfontfx.setColour(veccol4f(tm->getInt("R"), tm->getInt("G"),
                                     tm->getInt("B"),
  ...
  if (entrylength > thcfg->sub("ints")->getInt("menumaxtextsellength")) {
    strncpy(striptext, thistext.c_str(),
            thcfg->sub("ints")->getInt("menumaxtextsellength"));
    striptext[thcfg->sub("ints")->getInt("menumaxtextsellength")] = '\0';
    strcat(striptext, "...");
  }

  rtcfg->sub("objects")->sub("menu")->set("width",
                                          menuselfontfx.getWidth(striptext));

  if (thcfg->sub("ints")->getInt("menualign") == 2) {
    glTranslatef(rtcfg->sub("objects")->sub("menu")->getInt("maxwidth") -
                     rtcfg->sub("objects")->sub("menu")->getFloat("width"),
                 0, 0);
  }  // right
  ...
  menuselfontfx.draw(0.0, 0.0, striptext,
                     thcfg->sub("objects")->sub("menu")->getFloat("depth"));
}

void drawmenutext(std::string thistext, int showid) {
  if (thcfg->sub("objects")
          ->sub(thcfg->sub("objects")->getKey(showid))
          ->getBool("enabled") ||
      pick() == 4) {
    char striptext[500];
    int entrylength = strlen(thistext.c_str());
    ...
    if (entrylength > thcfg->sub("ints")->getInt("menumaxtextlength")) {
      strncpy(striptext, thistext.c_str(),
              thcfg->sub("ints")->getInt("menumaxtextlength"));
    ...
    rtcfg->sub("objects")
        ->sub(thcfg->sub("objects")->getKey(showid))
        ->set("width", font.getWidth(striptext));
...
void drawtitletext(std::string thistext) {
  ...
  {
    titlefontfx.draw(
        thcfg->sub("objects")->sub("title")->getInt("maxtitlelength") -
            rtcfg->sub("objects")->sub("title")->getFloat("width"),
        0.0, thistext, tdepth);
  }
...
void drawsettingstext(std::string thistext, bool align) {
  if (align)
    settingsfontfx.draw(
        thcfg->sub("objects")->sub("setbox")->getInt("width") / 2 -
            settingsfontfx.getWidth(thistext),
        0.0, thistext, 0.0);
  else
    settingsfontfx.draw(0.0, 0.0, thistext, 0.0);
}

void drawrsstext(std::string thistext) {
  textcol("rss", &rssfontfx, 0);
  textrot("rss", thistext, &rssfontfx, 0);
  rssfontfx.draw(0.0, 0.0, thistext,
                 thcfg->sub("objects")->sub("rss")->getFloat("depth"));
}

void updatefont() {
  rtcfg->set("ttffontfile", rtcfg->sub("scanned")->getString("fontpath") + "/" +
                                thcfg->sub("files")->getString("fontfile"));
...
void changemaxtextlength(int by, std::string thislength) {
  ...
  if (rtcfg->getBool("initdone"))
    shownote(lacfg->getString("changed") + thislength + lacfg->getString("to") +
             itos(thcfg->sub("ints")->getInt(thislength)));
}

void resizemusfont() {
  musicdurationfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("sound")->getInt("size")));
}

void resizetitfont() {
  titlefontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("title")->getInt("size")));
}

void resizeclofont() {
  clockfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("clock")->getInt("size")));
}
void resizedatfont() {
  datefontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("date")->getInt("size")));
}

void resizesysfont() {
  sysinfofontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("cpu")->getInt("size")));
}

void resizenotfont() {
  notifyfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("notifier")->getInt("size")));
}

void resizerssfont() {
  rssfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("rss")->getInt("size")));
}

void resizeromfont() {
  counterfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("entrycount")->getInt("size")));
}

void resizerominfofont() {
  rominfofontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("platform")->getInt("size")));
}

void resizesetfont() {
  settingsfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("setbox")->getInt("size")));
}

void resizemenfont() {
  font = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(thcfg->sub("objects")->sub("menu")->getInt("size")));
  std::string selwidth;
  for (int i = 0; i < thcfg->sub("ints")->getInt("menumaxtextlength"); i++) {
    selwidth += "W";
  }
  rtcfg->set("maxmenuselwidth", (int)font.getWidth(selwidth));
  if (rtcfg->getBool("initdone")) {
    if (rtcfg->getInt("maxmenuselwidth") >
        rtcfg->sub("objects")->sub("menu")->getInt("maxwidth")) {
      changemaxtextlength(-1, "menumaxtextlength");
    }
    if ((int)font.getWidth(selwidth + "W") <
        rtcfg->sub("objects")->sub("menu")->getInt("maxwidth")) {
      changemaxtextlength(+1, "menumaxtextlength");
    }
  }
...
void resizeselfont() {
  menuselfontfx = fontmanager.grab(
      rtcfg->getString("ttffontfile"),
      drawfontsize(
          thcfg->sub("objects")->sub("menu")->getInt("menuselfontsize")));

  std::string selwidth;
  for (int i = 0; i < thcfg->sub("ints")->getInt("menumaxtextsellength"); i++) {
    selwidth += "W";
  }
  ...
  if ((int)menuselfontfx.getWidth(selwidth + "W") <
      rtcfg->sub("objects")->sub("menu")->getInt("maxwidth")) {
    changemaxtextlength(+1, "menumaxtextsellength");
  }
}
