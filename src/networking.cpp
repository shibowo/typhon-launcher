...
void getrominfo(int e, int r, int m) {
  ...
  {
    bool alreadyhere = 0;
    rtcfg->set("currententry",
               rmcfg->sub("Roms")->sub(e)->sub(r)->getString("file"));
    ...
    if (check) {
      ...
      if (tycfg->sub("globalswitches")->getBool("useinternet")) {
        std::string search =
            noVersion(rtcfg->getString("currententry")).c_str();  // cut version
        ...
        for (int i = 0; i < rmcfg->sub("striptitle")->size(); i++) {
          search =
              cutstring(search.c_str(), rmcfg->sub("striptitle")->getString(i));
        }

        downloadurito(tycfg->sub("strings")->getString("scraperhost") + "/" +
                          tycfg->sub("strings")->getString("scraperuri") +
                          "?what=" + search,
                      rominfo);
        prcfg->wipe();
    ...
    if (!alreadyhere && prcfg->sub("games")->exists(0)) {
      if (prcfg->sub("games")->sub(0)->exists("title")) prcfg->saveXML(rominfo);
    }
  }
}
...
void getproginfo() {
  if (tycfg->sub("globalswitches")->getBool("useinternet")) {
    ...
    std::string temp =
        tycfg->sub("tempdirs")->getString("import") + "/" + search + ".xml";
    downloadurito(tycfg->sub("strings")->getString("scraperhost") + "/" +
                      tycfg->sub("strings")->getString("scraperuri") +
                      "?what=" + search,
                  temp);
    ...
    if (prcfg->sub("games")->size() > 0) {
      std::string pic = cutstring(
          prcfg->sub("games")->sub(0)->sub("images")->sub("boxart")->getString(
              0),
          "http://");
      if (compare(pic, "back"))
        pic = cutstring(prcfg->sub("games")
                            ->sub(0)
                            ->sub("images")
                            ->sub("boxart")
                            ->getString(1),
...
std::string boxart(int e, int r, int num) {
  return prcfg->sub("games")
      ->sub(rmcfg->sub("Roms")->sub(e)->sub(r)->getInt("gameid"))
      ->sub("images")
      ->sub("boxart")
      ->getString(num);
}
...
void downortextprev(int e, int r) {
  ...
  std::string rominfo = rtcfg->getString("rominfodir") + "/" + itos(e) + "/" +
                        getPrefix(rtcfg->getString("currententry")) + ".xml";
  ...
  for (int i = 0;
    ...
    if (compare(boxart(e, r, i), "http://")) {
      ...
      if (tycfg->sub("globalswitches")->getBool("useinternet")) {
        ...
        if (compare(boxart(e, r, i), "front")) {
          ...
          if (check)
            ...
            downloadurito(cutstring(boxart(e, r, i), "http://"), file);
        ...
        if (prcfg->sub("games")->exists(0)) {
          if (prcfg->sub("games")->sub(0)->exists("title"))
            prcfg->saveXML(rominfo);
        }
      } else {
      ...
      else if (compare(boxart(e, r, i), "back") &&
               thcfg->sub("objects")->sub("preview")->getBool("back")) {
...
void downloadrompreview(int e, int r) {
  ...
  if (prcfg->sub("games")->size() > 0) {
    if (rmcfg->sub("Roms")->sub(e)->sub(r)->getInt("gameid") >
        prcfg->sub("games")->size() - 1)
