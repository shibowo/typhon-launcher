...
void toggleshaders() {
  if (useShader() == 0)
    thcfg->sub("shader")->setBool("useshader", 1);
  else
    thcfg->sub("shader")->setBool("useshader", 0);
}

int switchshadertex(std::string thistexture, sf::Texture *texi) {
  if ((strcmp(rtcfg->getString("oldshadertex").c_str(), thistexture.c_str()) !=
       0)) {
...
void updateshaders(ShaderManager &shaderman) {
  ...
  if (sh->getBool("mouse")) {
    shaderman.getShader(useBackShader())
        .setParameter("mouse", (rootX), (rootY), (rootX), (rootY));
  }

  if (sh->getBool("size"))
    shaderman.getShader(useBackShader())
        .setParameter("resolution", (rtcfg->getInt("width")),
                      (rtcfg->getInt("height")));

  if (sh->getBool("time")) {
    //		if (rtcfg->getFloat("shcount")<30)
    ((TConfig<float> *)rtcfg->get("shcount"))
        ->operator+(thcfg->sub("floats")->getFloat("shaderspeed"));
    //		else rtcfg->set("shcount",(float)0);
    shaderman.getShader(useBackShader())
        .setParameter("time", rtcfg->getFloat("shcount"));
  }

  if (sh->getBool("color")) {
    TyphonConfig *mc =
        rtcfg->sub("monthcolors")->sub(rtcfg->getInt("monthcolor"));
    rtcfg->setBool("wantshadercolor", 1);
    clocktimer();
    rtcfg->setBool("wantshadercolor", 0);
    shaderman.getShader(useBackShader())
        .setParameter("color", mc->getFloat("R"), mc->getFloat("G"),
                      mc->getFloat("B"));
  }
}
...
void addusershaders() {
  ...
  if ((num_entries = scandir((usershaderpath).c_str(), &sdit, fileselectAll,
                             alphasort)) < 0) {
  } else {
    for (int j = 0; j < num_entries; j++) {
      if (strstr(sdit[j]->d_name, ".sfx")) {
        ...
        for (int s = 0; s < shcfg->sub("shaders")->size() - 1; s++) {
          if (compare(sdit[j]->d_name,
                      shcfg->sub("shaders")->sub(s)->getString("name")))
            addnew = 0;
        }
        ...
        if (addnew) {
          ...
          TyphonConfig *sh =
              shcfg->sub("shaders")->sub(getPrefix(sdit[j]->d_name));
          shcfg->sub("shaders")
              ->subC(getPrefix(sdit[j]->d_name))
              ->set("name", sdit[j]->d_name);
...
void loadShader(int shadid, ShaderManager &shaderman) {
  std::string shaderfile =
      shcfg->sub("shaders")->sub(shadid)->getString("name");
  std::string sp = rtcfg->getString("systemshaderpath") + shaderfile;
  FILE *shcheck = fopen(sp.c_str(), "r");
  if (shcheck) {
    fclose(shcheck);
  } else {
    sp = checkpath(shcfg->getString("shaderpath"),
                   rtcfg->getString("systemshaderpath")) +
         shaderfile;
  }
...
void loadShaderConfig() {
  FILE *shcfgcheck =
      fopen(rtcfg->sub("configs")->getString("shaderxml").c_str(), "r");
  if (shcfgcheck) {
...
void shaderwp(std::string thiswallpaper) {
  std::string thisshadertex =
      rtcfg->sub("scanned")->getString("wallpaperpath") + "/" + thiswallpaper;

  FILE *shadcheck = fopen(thisshadertex.c_str(), "r");
  if (!shadcheck) {
    thisshadertex = rtcfg->sub("scanned")->getString("wallpaperpath") + "/" +
                    thcfg->sub("textures")->getString("preview");
  }
...
void loadshadertex() {
  if (shcfg->sub("shaders")->sub(useBackShader())->getBool("texture") &&
      thcfg->sub("shader")->getBool("useshader")) {
    if (thcfg->sub("ints")->getInt("shadertexture") == 1)  // preview texture
    {
      switchshadertex(rtcfg->getString("currentprev"), &shadtexture);
    }

    else if (thcfg->sub("ints")->getInt("shadertexture") ==
             2)  // wallpaper texture
    {
