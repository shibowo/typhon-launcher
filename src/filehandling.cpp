...
bool grep(std::string filename, std::string keyword) {
  ...
  while (getline(filecheck, searchline)) {
    if (!filecheck) break;
    if (searchline.find(keyword, 0) != string::npos) return 1;
  }
  filecheck.close();
  return 0;
}
...
void copyrecfromto(std::string orgdatadir, std::string fexportdir) {
  ...
  if ((num_entries = scandir((orgdatadir).c_str(), &sdit, fileselectAll,
                             alphasort)) < 0) {
  } else {
...
std::string setEnvhome(std::string str) {
  int pos = str.find("~");
  if (pos != -1)
    return getenv("HOME") + str.substr(pos + 1);
  else
    return str;
}
...
int clear(const char *file, const struct stat *sb, int flag) {
  remove(file);
  return 0;
}
...
void systemstart(std::string prog, std::string args, bool wait) {
  ...
  int start = 0;
  start =
      system(fullstart.c_str());  // maybe replace system with execvp one day
  if (start == -1) {
    printf("WARNING - can't start %s\n", fullstart.c_str());
  }
}
...
std::string fallbackmedia(std::string thismedia, int type) {
  ...
  if (prevcheck) {
    ...
    if (type == 1) {
      thismedia = rtcfg->sub("scanned")->getString("menuiconpath") + "/" +
                  thcfg->sub("textures")->getString("icon");
    }

    if (type == 2 || type == 3) {
      thismedia = rtcfg->sub("scanned")->getString("previewpath") + "/" +
                  gTex("preview");
    }
    ...
    if (prevcheck) {
      ...
      if (prevcheck) {
        ...
        if (type == 2 || type == 3)
          thismedia += rtcfg->getString("previewdefault");
      }
    }
  }
  return thismedia;
...
bool compare(std::string check1, std::string check2) {
  // strcasestr not in standard - therefore this function to not spread it over
  // the whole code
...
void togentryname(bool off) {
  if (off) {
    rtcfg->setBool("tempentrynames",
                   tycfg->sub("globalswitches")->getBool("entrynames"));
    tycfg->sub("globalswitches")->setBool("entrynames", 0);
  } else {
    tycfg->sub("globalswitches")
        ->setBool("entrynames", rtcfg->getBool("tempentrynames"));
  }
}
