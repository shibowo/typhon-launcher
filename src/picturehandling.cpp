...
extern std::string switchmediasuffix(
    std::string thismedia, TyphonConfig *suflist);  // from filehandling.cpp

using namespace std;

sf::Texture *getTexture(int s) {
  return rtcfg->sub("textureids")
      ->sub(rtcfg->sub("textureids")->sub(s)->id)
      ->getTexturePtr("tid");
}

sf::Texture *getIconTexture(int s) {
  return rtcfg->sub("textureids")
      ->sub(rtcfg->sub("textureids")
                ->sub(thcfg->sub("textures")->get("menuicon0")->id + s)
                ->id)
      ->getTexturePtr("tid");
}
...
sf::Texture *getWallTexture(int s) {
  return rtcfg->sub("textureids")
      ->sub(rtcfg->sub("textureids")
                ->sub(thcfg->sub("textures")->get("wallpaper0")->id + s)
                ->id)
      ->getTexturePtr("tid");
}
...
void texcol(std::string thistex) {
  if (rtcfg->getInt("picked") == thcfg->sub("objects")->get(thistex)->id &&
      pick() != 3 && pick() != 4) {
    ...
    if (pick() == 4 &&
        !thcfg->sub("objects")->sub(thistex)->getBool("enabled")) {
      glColor4ub(127, 127, 127, 127);
    } else {
      if (thcfg->sub("switches")->getBool("usemonthcolors")) {
        TyphonConfig *col =
            rtcfg->sub("monthcolors")->sub(rtcfg->getInt("monthcolor"));

        glColor4f(col->getFloat("R"), col->getFloat("G"), col->getFloat("B"),
                  norm255(thcfg->sub("objects")->sub(thistex)->getInt("A")));
      } else {
        TyphonConfig *col = thcfg->sub("objects")->sub(thistex);
        glColor4ub(col->getInt("R"), col->getInt("G"), col->getInt("B"),
                   col->getInt("A"));
      }
    }
  }
  ...
  if (Image.loadFromFile(thisimgpath)) {
    ...
    if (fcheck) {
      ...
      if (Image.loadFromFile(thisimgpath))
        ...
        rtcfg->set("currentprev",
                   fallbackmedia(rtcfg->getString("currentprev"), 2));
      }
    }
  }
#endif
...
void listpics() {
  ...
  listpictures("stencil",
               rtcfg->sub("scanned")->getString("stencilpicturepath"));
...
void loadmenuwallpapers(int thiswp) {
  if (!rtcfg->getBool("nop3tdata")) {
    ...
    updatewallpaper(thcfg->sub("textures")->getStringP(wpcat),
                    getTextureS(wpcat));
  }
}
...
void initpictureids() {
  for (int t = 0; t < thcfg->sub("textures")->size(); t++) {
    Config *tempcfg = getTextureConfig();
    rtcfg->sub("textureids")
        ->subC(thcfg->sub("textures")->getKey(t))
        ->set("tid", tempcfg);
  }
}
...
void updateimage(std::string path, std::string img, std::string imgback) {
  if (thcfg->sub("objects")->sub(img)->getBool("enabled")) {
    updateimage(rtcfg->sub("scanned")->getString(path),
                thcfg->sub("textures")->getString(img), getTextureS(img));
    if (thcfg->sub("objects")->sub(img)->getBool("back"))
      updateimage(rtcfg->sub("scanned")->getString(path),
                  thcfg->sub("textures")->getString(imgback),
                  getTextureS(imgback));
  }
}
...
void updateimages() {
  ...
  if (!thcfg->sub("switches")->getBool("usemenuwallpaper"))
    updatewallpaper(thcfg->sub("textures")->getStringP("wallpaper"),
                    getTextureS("wallpaper"));
  updateimage(rtcfg->sub("scanned")->getString("settingspicturepath"),
              thcfg->sub("textures")->getString("settings"),
              getTextureS("settings"));
  updatepreview();
  ...
  if (thcfg->sub("switches")->getBool("usemenuwallpaper")) {
    for (int i = 0; i < 9; i++) {
      loadmenuwallpapers(i);
    }
  }

  if (thcfg->sub("objects")->sub("wallpaper")->getBool("enabled") &&
      thcfg->sub("objects")->sub("wallpaper")->getBool("back"))
    updateimage(rtcfg->sub("scanned")->getString("wallpaperpath"),
                thcfg->sub("textures")->getString("wallpaperback"),
                getTextureS("wallpaperback"));
...
// thanks a lot to ehamberg (http://hamberg.no/erlend) for lending me his
// screenshot code from https://gist.github.com/767824
#ifdef WITHSCROT
int write_jpeg(XImage *img, const char *filename) {
  ...
  while (cinfo.next_scanline < cinfo.image_height) {
    row_pointer =
        (JSAMPROW)&buffer[cinfo.next_scanline * (img->depth >> 3) * img->width];
...
void takescreenshot() {
  ...
  xImageSample = XGetImage(display, DefaultRootWindow(display), 0, 0,
...
void updateimage(std::string thisscannedpath, std::string thisimage,
                 sf::Texture *thisimageID) {
...
void updatewallpaper(std::string *thiswallpaper, sf::Texture *thisbackID) {
  ...
  rtcfg->set("currentback", rtcfg->sub("scanned")->getString("wallpaperpath") +
                                "/" + *thiswallpaper);
  ...
  if (backcheck) {
    ...
    if (isp3t() && !rtcfg->getBool("emptyp3t")) {
      rtcfg->setBool("nop3tdata", 1);
      thcfg->sub("shader")->set("usebackshader",
                                shcfg->sub("shaders")->sub("xmb")->id);
    } else {
      rtcfg->set("currentback",
                 rtcfg->sub("scanned")->getString("wallpaperpath") + "/" +
                     gTex("preview"));
    }
  }
  ...
  if (!rtcfg->getBool("nop3tdata")) {
    ...
    if (backcheck) {
      ...
      rtcfg->set("currentback", rtcfg->getString("datadir") + "/" +
                                    rtcfg->getString("previewdefault"));
    }
  ...
  if (thcfg->sub("objects")->sub("icon")->getBool("enabled")) {
    if (Iem() == 0 || Ishowmenu() == 0) {
      if (mecfg->sub(Ishowmenu() - 1)->sub("entry")->sub(Ics())) {
        rtcfg->set("currenticon", mecfg->sub(Ishowmenu() - 1)
                                      ->sub("entry")
                                      ->sub(Ics())
                                      ->getString("icon"));
      } else {  // assume "empty" menu - typhon broke the menuentry again
        std::string default_icon =
            rtcfg->sub("scanned")->getString("menuiconpath") + "/" +
            thcfg->sub("textures")->getString("icon");
        default_icon += rtcfg->sub("scanned")->getString("menuiconpath") + "/" +
                        thcfg->sub("textures")->getString("icon");
    ...
    if (Ishowmenu() >= 1 && Iem() != 0) {
      rtcfg->set("currenticon", emcfg->sub(IeID() - 1)->getString("rompath") +
                                    "/" +
                                    getPrefix(rmcfg->sub("Roms")
                                                  ->sub(IeID() - 1)
                                                  ->sub(Ics())
                                                  ->getString("file")) +
                                    ".png");
    ...
    if (iconcheck) {
      ...
      rtcfg->set("currenticon", setEnvhome(rtcfg->getString("currenticon")));
    }

    rtcfg->set("currenticon",
               switchmediasuffix(rtcfg->getString("currenticon"),
                                 rtcfg->sub("suffix")->sub("pics")));
...
void loadmenuicon(std::string iconmenu, sf::Texture *thisID) {
  rtcfg->set("currentmenuicon",
             rtcfg->sub("scanned")->getString("menuiconpath") + "/" + iconmenu);
  FILE *micheck = fopen(rtcfg->getString("currentmenuicon").c_str(), "r");
  if (micheck) {
    fclose(micheck);
  } else {
    rtcfg->set("currentmenuicon",
               fallbackmedia(rtcfg->getString("currentmenuicon"), 1));
  }
...
void loaddefmenuicons(int thisicon) {
  ...
  loadmenuicon(thcfg->sub("textures")->getString(iconcat),
               getTextureS(iconcat));
}

void loademumenuicon(std::string thisemu, int thisemuicon,
                     sf::Texture *thisID) {
  rtcfg->set(
      "currentmenuicon",
      rtcfg->sub("scanned")->getString("previewpath") + "/" + thisemu + ".png");
  ...
  if (micheck) {
    ...
    rtcfg->set("currentmenuicon",
               switchmediasuffix(rtcfg->getString("currentmenuicon"),
                                 rtcfg->sub("suffix")->sub("pics")));
    FILE *micheck = fopen(rtcfg->getString("currentmenuicon").c_str(), "r");
    if (micheck) {
      fclose(micheck);
    } else {
      loaddefmenuicons(thisemuicon);
    }
  }
  updatetexture(rtcfg->getString("currentmenuicon"), thisID);
}
...
void loadmenuicons() {
  loadmenuicon(thcfg->sub("textures")->getString("menuicon0"),
               getTextureS("menuicon0"));

  for (int m = 1; m < mecfg->size() + 1; m++) {
    int thismiconid = thcfg->sub("textures")->get("menuicon0")->id + m;
    std::string iconcat = thcfg->sub("textures")->getKey(thismiconid);
    if (mecfg->sub(m - 1)->getInt("emumenu") == 0) {
      loadmenuicon(thcfg->sub("textures")->getString(iconcat),
                   getTextureS(iconcat));
    }
    if (mecfg->sub(m - 1)->getInt("emumenu") == 1) {
      loademumenuicon(emulator(m), m, getTextureS(iconcat));
    }
    if (mecfg->sub(m - 1)->getInt("emumenu") == 2) {
      loademumenuicon(emulator(m + mecfg->size()), m, getTextureS(iconcat));
    }
    if (mecfg->sub(m - 1)->getInt("emumenu") == 3) {
      loademumenuicon(emulator(m + (mecfg->size() * 2)), m,
                      getTextureS(iconcat));
    }
  }
  loadmenuicon(thcfg->sub("textures")->getString("menuicon8"),
               getTextureS("menuicon8"));
}
...
void preppreview(std::string preppreviewpath) {
  if (Bvidmenu()) {
    rtcfg->set(
        "currententry",
        getPrefix(vicfg->sub("videos")->sub(Ics())->getString("file").c_str()));
    rtcfg->set(
        "currentprevpath",
        setEnvhome(thcfg->sub("paths")->getString("videopreviewpath")) + "/");
    rtcfg->set("currentprev", rtcfg->getString("currentprevpath") +
                                  rtcfg->getString("currententry") + ".png");
  }
    ...
    if (Iem() != 0) {
      rtcfg->set("currententry", getPrefix(rmcfg->sub("Roms")
                                               ->sub(IeID() - 1)
                                               ->sub(Ics())
                                               ->getString("file")));
      rtcfg->set("currentprevpath",
                 emcfg->sub(IeID() - 1)->getString("rompicturespath") + "/");
...
void updatepreview() {
  if ((thcfg->sub("objects")->sub("preview")->getBool("enabled")) ||
      (thcfg->sub("ints")->getInt("shadertexture") == 1 &&
       useBackShader() != 0 && useShader()) ||
      rtcfg->getBool("videofailed")) {
    ...
    if (prevcheck) {
      ...
      rtcfg->set("currentprev", setEnvhome(rtcfg->getString("currentprev")));
    }
    rtcfg->set("currentprev",
               switchmediasuffix(rtcfg->getString("currentprev"),
                                 rtcfg->sub("suffix")->sub("pics")));

    if (Bp3tmenu()) {
      extractp3tpreview();
      rtcfg->set("currentprev", tycfg->sub("tempdirs")->getString("p3text") +
                                    "/info_preview.png");
      loadshadertex();
    }
    if (Bthememenu()) {
      if (strcmp(getPrefix(rtcfg->sub("themes")->getString(Ics())).c_str(),
                 "typhon") == 0)
        rtcfg->set("currentprev",
                   rtcfg->getString("datadir") + "/themepreview.jpg");
      else if (strcmp(getPrefix(rtcfg->sub("themes")->getString(Ics())).c_str(),
                      "p3t") == 0)
        rtcfg->set("currentprev",
                   rtcfg->getString("datadir") + "/p3tpreview.jpg");
      else
        rtcfg->set("currentprev",
                   rtcfg->sub("scanned")->getString("themedata") + "/" +
                       getPrefix(rtcfg->sub("themes")->getString(Ics())) +
                       "/themepreview.jpg");
    ...
    if (prevcheck) {
      ...
      if (rtcfg->getBool("nop3tdata")) {
        rtcfg->set("currentprev", rtcfg->getString("currentmenuicon"));
      } else {
        if (Iem() != 0)
          downloadrompreview(IeID() - 1, Ics());
        else
          rtcfg->set("currentprev",
                     fallbackmedia(rtcfg->getString("currentprev"), 2));
      }
    ...
    if (thcfg->sub("objects")->sub("preview")->getBool("back")) {
      ...
      updatetexture(rtcfg->getString("currentprevback"),
                    getTextureS("previewback"));
    }
  }
}
...
void switchpictureeffects(int thiseffect, float thisblend,
                          sf::Texture *thispicID, float thiswidth,
                          float thisheight) {
  switch (thiseffect) {
    ...
    case 4:  // fly to the left and blend
      glColor4f(1, 1, 1, thisblend);
      glTranslatef(-rtcfg->sub("objects")->sub("slide2")->getFloat("height") /
                       2 * (1 - thisblend),
                   0, 50.1f);
      sf::Texture::bind(thispicID);
      drawquad(thiswidth, thisheight);
    ...
    case 6:  // fly down and blend
      glColor4f(1, 1, 1, thisblend);
      glTranslatef(0,
                   -rtcfg->sub("objects")->sub("slide2")->getFloat("height") /
                       2 * (1 - thisblend),
                   50.1f);
      sf::Texture::bind(thispicID);
      drawquad(thiswidth, thisheight);
...
void drawback(sf::Texture *oldID, sf::Texture *newID) {
  ...
  if (rtcfg->getInt("picked") == thcfg->sub("objects")->get("wallpaper")->id &&
      ...
      rtcfg->sub("block")->getBool(
          thcfg->sub("objects")->get("wallpaper")->id)) {
    glColor4ub(1, 1, 255, 255);
  } else {
  ...
  if (!rtcfg->getBool("scrollisdone") && !rtcfg->getBool("changemenu")) {
    ((TConfig<float> *)rtcfg->get("blendwall"))
        ->operator-(rtcfg->getFloat("elapsed"));
    switchpictureeffects(
        thcfg->sub("ints")->getInt("wallpaperblendeffect"),
        rtcfg->getFloat("blendwall"), oldID,
        rtcfg->sub("objects")->sub("wallpaper")->getFloat("width"),
        rtcfg->sub("objects")->sub("wallpaper")->getFloat("height"));
  }
...
void iconcube(float fmicon, int thisicon) {
  drawcube(rtcfg->sub("objects")->sub("menuicon")->getFloat("width") * fmicon,
...
void drawsinglemenuicon(int thisicon) {
  ...
  if (Ishowmenu() == thisicon) {
    ...
    glTranslatef(-((fmicon * 58.0 - 48.0) * icwidth),
                 ((fmicon * 58.0 - 48.0) * icheight), 0.0);
  };
  if (Ishowmenu() > thisicon) {
  ...
  if (thcfg->sub("ints")->getInt("menuicondockstyle") != 0) {
    if (thisicon == Ishowmenu()) {
      glScalef(rtcfg->sub("scaleicon")->getFloat("icon0"),
               rtcfg->sub("scaleicon")->getFloat("icon0"),
               rtcfg->sub("scaleicon")->getFloat("icon0"));
    }
    if (thisicon == Ishowmenu() - 1 || thisicon == Ishowmenu() + 1) {
      glScalef(rtcfg->sub("scaleicon")->getFloat("icon1"),
               rtcfg->sub("scaleicon")->getFloat("icon1"),
               rtcfg->sub("scaleicon")->getFloat("icon1"));
    }
    if (thisicon == Ishowmenu() - 2 || thisicon == Ishowmenu() + 2) {
      glScalef(rtcfg->sub("scaleicon")->getFloat("icon2"),
               rtcfg->sub("scaleicon")->getFloat("icon2"),
               rtcfg->sub("scaleicon")->getFloat("icon2"));
    }
  }
  rotateme("menuicon");

  if (thcfg->sub("ints")->getInt("menuicontype") == 2) {
    ...
    glTranslatef(
        ...
        thcfg->sub("objects")->sub("menuicon")->getFloat("depth") / 2);
    iconcube(fmicon, thisicon);
    if (rtcfg->getFloat("rotmenucube") >= 360)
      rtcfg->set("rotmenucube", (float)0);
  }
...
void drawmenuicons() {
  if (rtcfg->getBool("changemenu") && rtcfg->getInt("scrollmenu") == 1) {
    ((TConfig<float> *)rtcfg->get("scrollicon"))
        ->operator+(thcfg->sub("floats")->getFloat("menuiconscrollspeed"));
  }
  ...
  if (thcfg->sub("ints")->getInt("menuiconxmbmode") == 0) {
    ...
    for (int m = 0; m < mecfg->size() + 2; m++) {
      rtcfg->sub("ipos")->set(
          m, (float)(rtcfg->getFloat("showmiconspace") * (m - Ishowmenu() + 1) +
                     rtcfg->getFloat("scrollicon")));
      ...
      if (m > 0 || (m == 0 && tycfg->getBool("showemulist"))) {
        ...
        if (thcfg->sub("ints")->getInt("menuiconxmbmode") == 1)
          glTranslatef(
              rtcfg->sub("ipos")->getFloat(m) +
                  rtcfg->sub("objects")->sub("menuicon")->getFloat("posx") -
                  rtcfg->getFloat("showmiconspace"),
...
void drawwallpapers() {
  if (thcfg->sub("objects")->sub("wallpaper")->getBool("enabled") &&
      !thcfg->sub("objects")->sub("slide1")->getBool("enabled") &&
      !rtcfg->getBool("nop3tdata")) {
    if (thcfg->sub("switches")->getBool("usemenuwallpaper")) {
      int thiswpid =
          thcfg->sub("textures")->get("wallpaper0")->id + Ishowmenu();
      std::string wpcat = thcfg->sub("textures")->getKey(thiswpid);
      std::string prevwp;
      std::string nextwp;

      if (Ishowmenu() == 0) {
        prevwp = thcfg->sub("textures")
                     ->getKey(thcfg->sub("textures")->get("wallpaper0")->id +
                              mecfg->size() + 1);
        nextwp = thcfg->sub("textures")->getKey(thiswpid + 1);
      } else if (Ishowmenu() == mecfg->size() + 1) {
        prevwp = thcfg->sub("textures")->getKey(thiswpid - 1);
        nextwp == thcfg->sub("textures")
                      ->getKey(thcfg->sub("textures")->get("wallpaper0")->id);
      } else {
        prevwp = thcfg->sub("textures")->getKey(thiswpid - 1);
        nextwp = thcfg->sub("textures")->getKey(thiswpid + 1);
      ...
      /*
              if(rtcfg->getBool("wallscroll"))	{
                                                              if
         (tycfg->getBool("showemulist"))
         {drawback(getTexture(rtcfg->sub("textureids")->sub("wallpaper0")->id),getTexture(rtcfg->sub("textureids")->sub("wallpaper1")->id));}
                                                              else
         {drawback(getTexture(rtcfg->sub("textureids")->sub("wallpaper8")->id),getTexture(rtcfg->sub("textureids")->sub("wallpaper1")->id));}
                                                      }
      */
      if (thcfg->sub("switches")->getBool("fadewallpaper")) {
        if (rtcfg->getBool("wallscroll")) {
          drawback(getTexture(rtcfg->sub("textureids")->sub(prevwp)->id),
                   getTexture(rtcfg->sub("textureids")->sub(wpcat)->id));
        } else {
          drawback(getTexture(rtcfg->sub("textureids")->sub(nextwp)->id),
                   getTexture(rtcfg->sub("textureids")->sub(wpcat)->id));
        }
      } else
        drawback(getTexture(rtcfg->sub("textureids")->sub(wpcat)->id),
                 getTexture(rtcfg->sub("textureids")->sub(wpcat)->id));
    } else {
      drawback(getTexture(rtcfg->sub("textureids")->sub("wallpaper")->id),
...
void drawpictures() {
  ...
  if (thcfg->sub("objects")->sub("preview")->getBool("enabled") ||
      pick() == 4) {
    drawpic("preview", "previewback");
  }
  if (thcfg->sub("objects")->sub("previewreflection")->getBool("enabled") ||
      pick() == 4) {
    drawref("previewreflection");
  }
  if (thcfg->sub("objects")->sub("icon")->getBool("enabled") || pick() == 4) {
    drawpic("icon", "icon");
  }
  if (thcfg->sub("objects")->sub("stencil")->getBool("enabled") ||
      pick() == 4) {
    drawpic("stencil", "stencilback");
  }
  if (thcfg->sub("objects")->sub("menuicon")->getBool("enabled") ||
      pick() == 4) {
    drawmenuicons();
  }
  if (thcfg->sub("objects")->sub("custom1")->getBool("enabled") ||
      pick() == 4) {
    drawpic("custom1", "custom1back");
  }
  if (thcfg->sub("objects")->sub("custom2")->getBool("enabled") ||
      pick() == 4) {
    drawpic("custom2", "custom2back");
  }
  if (thcfg->sub("objects")->sub("custom3")->getBool("enabled") ||
      pick() == 4) {
    drawpic("custom3", "custom3back");
  }
}
...
void changemenuicon(bool up) {
  if (rtcfg->sub("icons")->size() > 0) {
    if (up) {
      if (rtcfg->getInt("currentmicon") == 0) {
        rtcfg->set("currentmicon", rtcfg->sub("icons")->size() - 1);
      } else {
        ((TConfig<int> *)rtcfg->get("currentmicon"))->operator-(1);
      }
    } else {
      if (rtcfg->getInt("currentmicon") == rtcfg->sub("icons")->size() - 1) {
        rtcfg->set("currentmicon", 0);
      } else {
        ((TConfig<int> *)rtcfg->get("currentmicon"))->operator+(1);
      }
    }
    ...
    int thismiconid =
        thcfg->sub("textures")->get("menuicon0")->id + Ishowmenu();
    std::string iconcat = thcfg->sub("textures")->getKey(thismiconid);
    thcfg->sub("textures")
        ->set(iconcat,
              rtcfg->sub("icons")
                  ->sub(rtcfg->getInt("currentmicon"))
                  ->getString("file"),
              thismiconid);
...
void changewallpaper(bool up) {
  if (rtcfg->sub("wallpapers")->size() > 0) {
    if (up) {
      if (rtcfg->getInt("currentwp") == 0) {
        rtcfg->set("currentwp", rtcfg->sub("wallpapers")->size() - 1);
      } else {
        ((TConfig<int> *)rtcfg->get("currentwp"))->operator-(1);
      }
    } else {
      if (rtcfg->getInt("currentwp") == rtcfg->sub("wallpapers")->size() - 1) {
        rtcfg->set("currentwp", 0);
      } else {
        ((TConfig<int> *)rtcfg->get("currentwp"))->operator+(1);
      }
    }
...
void changepicture(std::string listcat, std::string iconcat,
                   std::string counter, bool up) {
  if (rtcfg->sub(listcat)->size() > 0) {
    if (up) {
      if (rtcfg->getInt(counter) == 0) {
        rtcfg->set(counter, rtcfg->sub(listcat)->size() - 1);
      } else {
        ((TConfig<int> *)rtcfg->get(counter))->operator-(1);
      }
    } else {
      if (rtcfg->getInt(counter) == rtcfg->sub(listcat)->size() - 1) {
        rtcfg->set(counter, 0);
      } else {
        ((TConfig<int> *)rtcfg->get(counter))->operator+(1);
      }
    ...
    thcfg->sub("textures")
        ->setC(iconcat, rtcfg->sub(listcat)
                            ->sub(rtcfg->getInt(counter))
                            ->getString("file"));

    std::string picpath;
    if (strcmp(listcat.c_str(), "stencil") == 0)
      picpath = "stencilpicturepath";
    else
      picpath = "customimagepath";
    updateimage(rtcfg->sub("scanned")->getString(picpath),
                thcfg->sub("textures")->getString(iconcat),
                getTextureS(iconcat));
  }
}
...
void listpictures(std::string type, std::string searchpath) {
  struct dirent **sdit;
  int num_entries;
  if ((num_entries = scandir((searchpath).c_str(), &sdit, fileselectAll,
                             alphasort)) < 0) {
  } else {
    int k = 0;
    for (int j = 0; j < num_entries; j++) {
      if (compare(sdit[j]->d_name, ".png") ||
...
void changemenuiconspace() {
  ((TConfig<float> *)thcfg->sub("floats")->get("menuiconspace"))
      ->operator+(changex);
