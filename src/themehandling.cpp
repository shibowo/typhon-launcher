...
void changetheme() {
  ...
  if (rtcfg->getInt("themetype") == TYP && isp3t())
    rtcfg->setBool("nop3tdata", 1);
  else
    rtcfg->setBool("nop3tdata", 0);
...
void copytyphonthemedata() {
  ...
  std::string srcdir = tycfg->sub("tempdirs")->getString("import") + "/" +
                       tycfg->getString("themename") + "/themedata/" +
                       tycfg->getString("themename") + "/";
  std::string destdir = rtcfg->sub("scanned")->getString("themedata") + "/" +
                        tycfg->getString("themename");
  mkdir(destdir.c_str(), 0755);
  copyrecfromto(srcdir, destdir);

  // copy the theme config
  copyfromto(tycfg->sub("tempdirs")->getString("import") + "/" +
                 tycfg->getString("themename") + "/themes/" +
                 tycfg->getString("themename") + ".xml",
             rtcfg->getString("tythemedir"));
}
...
void exporttheme() {
  ...
  if (rtcfg->getInt("themetype") == PS3) {
    ...
    orgdatadir = rtcfg->sub("scanned")->getString("themedata") + "/" +
                 tycfg->getString("themename");
  }
  ...
  rtcfg->sub("configs")->set(
      "themexml", fexportdir + rtcfg->getString("exportname") + ".xml");
  ...
  if (!rtcfg->getBool("havethedate")) {
    clocktimer();
  }

  std::string exportcmd;
  exportcmd = tycfg->sub("tempdirs")->getString("export") + " && tar cfz " +
              exportdir + rtcfg->getString("exportname") + "-v" +
              rtcfg->getString("themeversion") + ".tgz " +
              rtcfg->getString("exportname");
...
void startexporttheme() {
  ...
  if (rtcfg->getInt("themetype") == PS3) {
    rtcfg->set("exportname", getPrefix(tycfg->getString("p3tfile")));
  } else {
    rtcfg->set("exportname", tycfg->getString("themename"));
  }
  editorinput(rtcfg->getString("exportname"), 3);
}

void createthemedatadir() {
  rtcfg->sub("scanned")->set("themedata",
                             tycfg->sub("paths")->getString("themedata"));
  FILE *pathcheck =
      fopen(rtcfg->sub("scanned")->getString("themedata").c_str(), "r");

  if (pathcheck) {
    fclose(pathcheck);
  } else {
    rtcfg->sub("scanned")->set(
        "themedata", setEnvhome(rtcfg->sub("scanned")->getString("themedata")));
  }
  ...
  if (pathcheck) {
    ...
    mkdir(rtcfg->sub("scanned")->getString("themedata").c_str(), 0755);
  }
}

void themepath(std::string themename) {
  rtcfg->sub("configs")->set("themexml", rtcfg->getString("tythemedir") +
                                             getPrefix(themename) + ".xml");
}

void savetheme() {
  FILE *wthemefile =
      fopen(rtcfg->sub("configs")->getString("themexml").c_str(), "w");
  if (wthemefile) {
    savethemexmlcfg();
    fclose(wthemefile);
    shownote(lacfg->getString("saved") + tycfg->getString("themename"));
  } else {
    shownote(lacfg->getString("cantwrite") +
             rtcfg->sub("configs")->getString("themexml"));
...
void readtheme() {
  FILE *themexmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("themexml").c_str(), "r");
  if (themexmlcfgcheck) {
    loadthemexmlcfg();
...
void themelist(bool up) {
  if (Ishowmenu() != 0) {
    if (up) {
      if (Ics() == 0) {
        setcs(rtcfg->sub("themes")->size() - 1);
      } else {
        setcs(Ics() - 1);
      }
    } else {
      if (Ics() == rtcfg->sub("themes")->size() - 1) {
        setcs(0);
      } else {
        setcs(Ics() + 1);
      }
...
void createthisthemedir() {
  if (rtcfg->getBool("importingtheme") && rtcfg->getInt("themetype") == PS3) {
    rtcfg->set("ithemedatadir",
               rtcfg->sub("scanned")->getString("themedata") + "/" +
                   getPrefix(tycfg->getString("p3tfile")) + "/");
  } else {
    rtcfg->set("ithemedatadir", rtcfg->sub("scanned")->getString("themedata") +
                                    "/" + tycfg->getString("themename") + "/");
  }
...
void extractzip() {
  std::string extargs = "-qq -j " + rtcfg->getString("importtheme") + " -d " +
                        tycfg->sub("tempdirs")->getString("import");
  systemstart("unzip", extargs, 1);
}

void extract7z() {
  std::string extargs = "e " + rtcfg->getString("importtheme") + " -o" +
                        tycfg->sub("tempdirs")->getString("import");
  systemstart("7za", extargs, 1);
}

void extractrar() {
  std::string extargs = "e -ep " + rtcfg->getString("importtheme") + " " +
                        tycfg->sub("tempdirs")->getString("import") + " -inul";
  systemstart("unrar", extargs, 1);
}

void extractgz() {
  std::string extargs = "xvfz " + rtcfg->getString("importtheme") + " -C " +
                        tycfg->sub("tempdirs")->getString("import");
  systemstart("tar", extargs, 1);
}

void extractbz2() {
  std::string extargs = "xvfj " + rtcfg->getString("importtheme") + " -C " +
                        tycfg->sub("tempdirs")->getString("import");
  systemstart("tar", extargs, 1);
}
...
void extracttheme() {
  ...
  if (rtcfg->getInt("themetype") == ZIP) {
    extractzip();
  }
  if (rtcfg->getInt("themetype") == RAR) {
    extractrar();
  }
  if (rtcfg->getInt("themetype") == SVNZ) {
    extract7z();
  }
  if (rtcfg->getInt("themetype") == GZ) {
    extractgz();
  }
  if (rtcfg->getInt("themetype") == TYPHON) {
    extractgz();
  }
  if (rtcfg->getInt("themetype") == BZ2) {
    extractbz2();
  }
}
...
int checkthemetype(std::string thischar) {
  ...
  if (suffix) {
    if (strcasecmp(suffix, ".p3t") == 0) {
      return PS3;
    }
    if (strcasecmp(suffix, ".tgz") == 0 && compare(thischar.c_str(), "-v")) {
      return TYPHON;
    }
    if (strcasecmp(suffix, ".jpg") == 0) {
      return JPG;
    }
    if (strcasecmp(suffix, ".gz") == 0) {
      return GZ;
    }
    if (strcasecmp(suffix, ".bz2") == 0) {
      return BZ2;
    }
    if (strcasecmp(suffix, ".zip") == 0) {
      return ZIP;
    }
    if (strcasecmp(suffix, ".rar") == 0) {
      return RAR;
    }
    if (strcasecmp(suffix, ".7z") == 0) {
      return SVNZ;
    } else
      printf("no valid theme - exiting!\n");
    exit(0);
    return TYP;
  } else
    printf("no valid theme - exiting!\n");
...
void themeimport() {
  ...
  rtcfg->sub("scanned")->set("wallpaperpath",
                             thcfg->sub("paths")->getString("wallpaperpath"));
  ...
  if (rtcfg->sub("wallpapers")->size() > 1) {
    sTex("stencil", rtcfg->sub("wallpapers")->sub(1)->getString("file"));
  }
  if (rtcfg->sub("wallpapers")->size() > 2) {
    sTex("custom1", rtcfg->sub("wallpapers")->sub(2)->getString("file"));
  }
  if (rtcfg->sub("wallpapers")->size() > 3) {
    sTex("custom2", rtcfg->sub("wallpapers")->sub(3)->getString("file"));
  }
  if (rtcfg->sub("wallpapers")->size() > 4) {
    sTex("custom3", rtcfg->sub("wallpapers")->sub(4)->getString("file"));
  } else {
  ...
  thcfg->sub("paths")->setC("stencilpicturepath",
                            rtcfg->getString("ithemedatadir"));
  thcfg->sub("paths")->setC("menuiconpath", rtcfg->getString("ithemedatadir"));
  thcfg->sub("paths")->setC("customimagepath",
                            rtcfg->getString("ithemedatadir"));
  themepath(tycfg->getString("themename"));
  savetheme();
  shownote(lacfg->getString("saved") + tycfg->getString("themename"));
...
void importtheme() {
  ...
  rtcfg->set("themetype",
             checkthemetype(getFile(rtcfg->getString("importtheme"))));
  ...
  if (themecheck) {
    ...
    if (rtcfg->getInt("themetype") == PS3) {
      rtcfg->sub("scanned")->set(
          "p3tpath", checkpath(tycfg->sub("paths")->getString("p3tpath"),
                               rtcfg->getString("datadir")));
      rtcfg->set("importtheme",
                 rtcfg->sub("scanned")->getString("p3tpath") + "/" +
                     getPrefix(rtcfg->getString("importtheme")) + ".p3t");
    }
    if (rtcfg->getInt("themetype") == TYPHON) {
      rtcfg->set("importtheme", rtcfg->getString("cfgdir") + "/export/" +
                                    getPrefix(rtcfg->getString("importtheme")) +
                                    ".tgz");
      FILE *themecheck = fopen(rtcfg->getString("importtheme").c_str(), "r");
      if (themecheck) {
        fclose(themecheck);
        ...
        for (int t = 0; t < oncfg->sub("themes")->size(); t++) {
          if (strcmp(getFile(oncfg->sub("themes")->getString(t)).c_str(),
                     getFile(rtcfg->getString("importtheme")).c_str()) == 0) {
  ...
  if (rtcfg->getInt("themetype") == PS3) {
    ...
    tycfg->set(
        "themename",
        getPrefix(getImportname(getFile(rtcfg->getString("importtheme")))));
    extracttheme();
    createthemedatadir();
    copytyphonthemedata();
  } else if (rtcfg->getInt("themetype") == ZIP ||
             rtcfg->getInt("themetype") == RAR ||
             rtcfg->getInt("themetype") == SVNZ ||
             rtcfg->getInt("themetype") == BZ2 ||
             rtcfg->getInt("themetype") == GZ) {
    tycfg->set("themename",
               getPrefix(getFile(rtcfg->getString("importtheme"))));
    extracttheme();
    themeimport();
    removerec(tycfg->sub("tempdirs")->getString("import"));
  }
...
void getonlinethemes(bool allowed) {
  if (tycfg->sub("globalswitches")->getBool("useinternet") || allowed) {
    downloadurito(tycfg->sub("strings")->getString("onlinethemes") + "/" +
                      rtcfg->sub("cfgs")->getString("online"),
                  rtcfg->sub("configs")->getString("onlinexml"));
    FILE *ocheck =
        fopen(rtcfg->sub("configs")->getString("onlinexml").c_str(), "r");
    if (ocheck) {
...
void gettheme() {
  downloadurito(
      ...
      rtcfg->getString("cfgdir") + "/export/" + "/" +
          getFile(oncfg->sub("themes")->getString(
              rtcfg->sub("helpsel")->getInt("online"))));

  FILE *tcheck = fopen((rtcfg->getString("cfgdir") + "/export/" + "/" +
                        getFile(oncfg->sub("themes")->getString(
                            rtcfg->sub("helpsel")->getInt("online"))))
                           .c_str(),
                       "r");
