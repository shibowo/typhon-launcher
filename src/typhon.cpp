...
int main(int argc, char* argv[]) {
  ...
  if (cli->exists("-lang")) {
    tycfg->sub("strings")->setC("language", cli->getString("-lang"));
  }
  ...
  if (cli->getBool("ar") || cli->getBool("-autores")) {
    tycfg->sub("globalswitches")
        ->setBoolC("autoresolution",
                   !tycfg->sub("globalswitches")->getBool("autoresolution"));
  }
  ...
  if (cli->getBool("l") || cli->getBool("-listthemes")) {
    cfgpath();
    printthemes(rtcfg->getString("tythemedir"), 1);
  }
  if (cli->getBool("lp") || cli->getBool("-listp3t")) {
    cfgpath();
    checkcfg();
    readtyphonxml();
    printp3ts();
  ...
  if (cli->exists("i")) {
    rtcfg->set("importtheme", cli->getString("i"));
    importtheme();
    exit(1);
  }
  if (cli->exists("-import")) {
    rtcfg->set("importtheme", cli->getString("-import"));
    importtheme();
    exit(1);
  }

  if (cli->getBool("le") || cli->getBool("-listexported")) {
    cfgpath();
    printthemes(rtcfg->getString("tyexportdir"), 0);
  }
  if (cli->getBool("lo") || cli->getBool("-listonline")) {
    cfgpath();
    checkcfg();
    readtyphonxml();
    printonlinethemes();
  ...
  if (cli->exists("fm") || cli->exists("-fillmenu")) {
    if (!cli->exists("m")) {
      printf("%s\n", lacfg->getString("missingarg").c_str());
      exit(0);
    } else {
      if (cli->getInt("m") < 1 || cli->getInt("m") > 7) {
        printf("%s\n", lacfg->getString("menurange").c_str());
        exit(0);
      } else {
        cfgpath();
        checkcfg();
        readtyphonxml();
        if (cli->exists("fm"))
          addentries(cli->getInt("m") - 1, cli->getString("fm"));
        else if (cli->exists("-fillmenu"))
          addentries(cli->getInt("m") - 1, cli->getString("-fillmenu"));
        savemenuxmlcfg();
        exit(0);
      }
    }
...
#ifdef WITHMMD
  listmmd(rtcfg->sub("scanned")->getString("modeldir"), ".pmd", "model",
          "models");
  listmmd(rtcfg->sub("scanned")->getString("motiondir"), ".vmd", "motion",
          "motion");
#endif  // WITHMMD
  ...
  if (tycfg->sub("globalswitches")->getBool("autoresolution") &&
      !(cli->getInt("x"))) {
    ...
    if (cli->getInt("x") && cli->getInt("y")) {
      rtcfg->set("width", cli->getInt("x"));
      rtcfg->set("height", cli->getInt("y"));
    }

    window.create(
        sf::VideoMode(rtcfg->getInt("width"), rtcfg->getInt("height"), 32),
        "typhon", sf::Style::None);
  }
  if (glewInit() != GLEW_OK) {
  ...
  unblock();
  loadShaderConfig();
  // I still wish I could just use a vector instead of a VLA but hey it works
  // now
  ...
  //	if(tycfg->sub("globalswitches")->getBool("useinputdaemon"))
  //closesocket();
  if (tycfg->sub("globalswitches")->getBool("saveconfigsonexit")) {
    saveglobalcfgs();
  }
