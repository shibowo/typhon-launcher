...
void listp3ts() {
  ...
  FILE *xmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("p3txml").c_str(), "r");
  if (xmlcfgcheck) {
#ifdef DEBUG
    ...
    int num_entries;
    if ((num_entries =
             scandir((rtcfg->sub("scanned")->getString("p3tpath")).c_str(),
...
void extractp3t() {
  if (strcmp((char *)(getPrefix(tycfg->getString("p3tfile")) + ".p3t").c_str(),
             "noneselected.p3t") != 0) {
    removerec(tycfg->sub("tempdirs")->getString("p3text"));
    rtcfg->set("themetype", PS3);
    std::string fullp3t;

    if (rtcfg->getBool("importingtheme")) {
      fullp3t = rtcfg->getString("importtheme");
    } else {
      fullp3t = rtcfg->sub("scanned")->getString("p3tpath") + "/" +
                getPrefix(tycfg->getString("p3tfile")) + ".p3t";
    }
    FILE *p3there = fopen(fullp3t.c_str(), "r");
    if (p3there) {
      fclose(p3there);
      p3textract(fullp3t.c_str(),
                 (char *)tycfg->sub("tempdirs")->getString("p3text").c_str());
...
void extractp3tpreview() {
  if (strcmp((char *)(getPrefix(tycfg->getString("p3tfile")) + ".p3t").c_str(),
             "noneselected.p3t") != 0) {
    std::string fullp3t = rtcfg->sub("scanned")->getString("p3tpath") + "/" +
                          getPrefix(tycfg->getString("p3tfile")) + ".p3t";
    p3tprevextract(fullp3t.c_str(),
                   (char *)tycfg->sub("tempdirs")->getString("p3text").c_str());
  }
}
...
void createthisthemebgdir() {
  ...
  if (fcheck) {
    ...
    mkdir(ibgdir.c_str(), 0755);
  }
}

// checks if p3t theme file is already in themedir and copies the default if it
// wasn't found
void checkp3ttheme() {
  ...
  if (themecheck) {
    ...
    std::string defaultp3tconf =
        rtcfg->getString("systemconfigdir") + "p3t.xml";
...
void copyp3t() {
  ...
  if ((num_entries =
           scandir(tycfg->sub("tempdirs")->getString("p3text").c_str(), &sdit,
                   fileselectAll, alphasort)) < 0) {
  } else {
    for (int j = 0; j < num_entries; j++) {
      if (compare(sdit[j]->d_name, ".png") ||
          compare(sdit[j]->d_name, ".jpg") || compare(sdit[j]->d_name, ".wav")

      ) {
        copyfromto(
            tycfg->sub("tempdirs")->getString("p3text") + "/" + sdit[j]->d_name,
  ...
  num_entries = 0;
  if ((num_entries =
           scandir(tycfg->sub("tempdirs")->getString("p3tebg").c_str(), &sdit,
                   fileselectAll, alphasort)) < 0) {
  } else {
    for (int j = 0; j < num_entries; j++) {
      if (compare(sdit[j]->d_name, ".jpg")) {
        copyfromto(
            tycfg->sub("tempdirs")->getString("p3tebg") + "/" + sdit[j]->d_name,
            rtcfg->getString("ithemedatadir") + "/backgrounds/");
      }
    }
  }
...
void switchp3t(bool up) {
  if (Ishowmenu() != 0) {
    if (up) {
      if (Ics() == 0) {
        setcs(p3cfg->sub("ps3themes")->size() - 1);
      } else {
        setcs(Ics() - 1);
      }
    } else {
      if (Ics() == p3cfg->sub("ps3themes")->size() - 1) {
        setcs(0);
      } else {
        setcs(Ics() + 1);
      }
...
bool isp3t() {
  if (strcmp((char *)tycfg->getString("themename").c_str(), "p3t") == 0)
    return 1;
  else
    return 0;
}
