...
void parsedesktopfiles() {
  ...
  struct dirent** sdit;
  int num_entries;
  if ((num_entries = scandir(desktopfiledir.c_str(), &sdit, fileselectAll,
                             alphasort)) < 0) {
  } else {
        ...
        if (desktopfile.is_open()) {
          while (desktopfile.good()) {
            ...
            if (line.find("Categories=", 0) != std::string::npos) {
              // games
              if ((line.find("Emulator;", 0) != std::string::npos &&
                   line.find("TerminalEmulator;", 0) == std::string::npos) ||
                  line.find("Game;", 0) != std::string::npos) {
                menu = 0;
              }
              // multimedia
              else if (line.find("AudioVideo", 0) != std::string::npos ||
              ...
              // internet
              else if (line.find("Network", 0) != std::string::npos ||
              ...
              // System
              else if (line.find("System", 0) != std::string::npos ||
              ...
              // System
              else if (line.find("Office", 0) != std::string::npos ||
                       line.find("TextTools", 0) != std::string::npos) {
                menu = 4;
              }
              // Graphics
              else if (line.find("Art;", 0) != std::string::npos ||
            ...
            if (line.find("Icon=", 0) != std::string::npos) {
              ...
              if (fExists(ticon)) {
                ...
                for (int p = 0; p < tycfg->sub("iconpaths")->size() - 1; p++) {
                  std::string pngpath = tycfg->sub("iconpaths")->getString(p) +
                                        "/" + ticon + ".png";
                  if (fExists(pngpath)) {
#ifdef WITHIM
                    sf::Image Image;
                    if (Image.loadFromFile(pngpath))
                      ...
                      icon = rtcfg->sub("scanned")->getString("iconconvpath") +
                             "/" + getPrefix(getFile(pngpath)) + ".png";
                  ...
                  std::string xpmpath = tycfg->sub("iconpaths")->getString(p) +
                                        "/" + ticon + ".xpm";
                  if (fExists(xpmpath)) {
#ifdef WITHIM
                    icon = rtcfg->sub("scanned")->getString("iconconvpath") +
                           "/" + getPrefix(getFile(xpmpath)) + ".png";
            ...
            if (line.find("Exec=", 0) != std::string::npos &&
                line.find("TryExec=", 0) == std::string::npos) {
          ...
          for (int e = 0; e < mecfg->sub(menu)->sub("entry")->size(); e++) {
            std::string old =
                mecfg->sub(menu)->sub("entry")->sub(e)->getString("name");
            if (strcmp(old.c_str(), name.c_str()) == 0 ||
                strcmp(old.c_str(), name.c_str()) == 0)
              exists = 1;
          }
          if (!exists) {
#ifdef DEBUG
...
void calcfademenu() {
  ...
  if (thcfg->sub("floats")->getFloat("menufadeduration") != fadespeed) {
    rtcfg->set("alphaspeed",
               (int)(255 / (thcfg->sub("floats")->getFloat("menufadeduration") /
                            fadespeed)) +
                   (int)fadespeed);
    if (rtcfg->getBool("changemenu") && rtcfg->getInt("scrollmenu") == 1) {
      ((TConfig<float>*)rtcfg->get("fademenu"))->operator+(fadespeed);
      ((TConfig<int>*)rtcfg->get("menualpha"))
          ->operator-(rtcfg->getInt("alphaspeed"));
    }
    ...
    if (thcfg->sub("switches")->getBool("invertfade")) {
      if (rtcfg->getFloat("oldfademenu") != 0 &&
          rtcfg->getInt("scrollmenu") == 1) {
        ((TConfig<float>*)rtcfg->get("oldfademenu"))->operator-(fadespeed);
        if (rtcfg->getFloat("oldfademenu") < 0)
          rtcfg->set("oldfademenu", (float)0);
      }
      if (rtcfg->getFloat("oldfademenu") != 0 &&
          rtcfg->getInt("scrollmenu") == 2) {
        ((TConfig<float>*)rtcfg->get("oldfademenu"))->operator+(fadespeed);
        if (rtcfg->getFloat("oldfademenu") > 0)
          rtcfg->set("oldfademenu", (float)0);
      }
  ...
  if ((rtcfg->getFloat("fademenu") >=
           thcfg->sub("floats")->getFloat("menufadeduration") ||
       rtcfg->getFloat("fademenu") <=
           -thcfg->sub("floats")->getFloat("menufadeduration")) ||
...
void drawentrycount(int curSel, int count) {
  ...
  if (thcfg->sub("objects")->sub("entrycur")->getBool("enabled")) {
    ...
    counterfontfx.draw(
        0.0, 0.0, itos(curSel),
        thcfg->sub("objects")->sub("entrycur")->getFloat("depth"));
    glPopMatrix();
  }
  // show maxcount
  if (thcfg->sub("objects")->sub("entrycount")->getBool("enabled")) {
    ...
    counterfontfx.draw(
        0.0, 0.0, itos(count - 1),
        thcfg->sub("objects")->sub("entrycount")->getFloat("depth"));
    glPopMatrix();
  }
}
...
void drawmenuentries() {
  if (Iem() != 0 && Ime() == 0) {
    printf("LISTROMS shouldn't happen here\n");
    listroms();
  }
  ...
  if (!thcfg->sub("switches")->getBool("hideaboveselection")) {
    for (z = 0; z > -(thcfg->sub("ints")->getInt("menulistmin") + 1); z--) {
      ...
      glPushMatrix();
      if (rtcfg->getFloat("oldfademenu") != 0)
        glTranslatef(rtcfg->getFloat("oldfademenu"), 0, 0);
      else
        glTranslatef(rtcfg->getFloat("fademenu"), 0, 0);
      if (z2 + Ics() != Ics()) {
        if (z2 <= -1 && z2 >= -(thcfg->sub("ints")->getInt("menulistmin"))) {
          ...
          if (rtcfg->getInt("menualpha") == 255)
            textcol(thcfg->sub("objects")->getKey(showminid), &font, 0);
          else
            textcol(thcfg->sub("objects")->getKey(showminid), &font,
                    rtcfg->getInt("menualpha"));
        }
  ...
  for (z = 1; z < thcfg->sub("ints")->getInt("menulistmax") + 1; z++) {
    ...
    if (Iem() != 0) {
      if (z2 + Ics() > rmcfg->sub("Roms")->sub(IeID() - 1)->size() - 1) {
        z2 = 0;
      };
    } else {
      if (z2 + Ics() > Ime() - 1) {
    ...
    if (z2 + Ics() != Ics()) {
      if (z2 >= 1 && z2 <= thcfg->sub("ints")->getInt("menulistmax")) {
        ...
        if (rtcfg->getInt("menualpha") == 255)
          textcol(thcfg->sub("objects")->getKey(showmaxid), &font, 0);
        else
          textcol(thcfg->sub("objects")->getKey(showmaxid), &font,
                  rtcfg->getInt("menualpha"));
      }
...
void drawrectbox() {
  ...
  glRecti(rtcfg->sub("objects")->sub("rectbox")->getFloat("width"),
          rtcfg->sub("objects")->sub("rectbox")->getFloat("height"), 0, 0);
...
void clocktimer() {
  ...
  if (thcfg->sub("objects")->sub("clock")->getBool("enabled")) {
    ...
    switch (thcfg->sub("ints")->getInt("clockformat")) {
      ...
      case 1: {
        ...
        strftime(clockbuf, 30, "%T", date);
        strcat(clockbuf, ".");
        strcat(clockbuf, msbuf);
        break;
      }
      case 2: {
        strftime(clockbuf, 30, "%I:%M %p", date);
        break;
      }
      case 3: {
        strftime(clockbuf, 30, "%I:%M:%S %p", date);
        break;
      }
      default:
        break;
  ...
  if (thcfg->sub("objects")->sub("date")->getBool("enabled") &&
      !rtcfg->getBool("havethedate")) {
    char datebuf[30];
    switch (thcfg->sub("ints")->getInt("dateformat")) {
      case 0: {
        strftime(datebuf, 40, "%d.%m.%Y", date);
        break;
      }
      case 1: {
        strftime(datebuf, 40, "%d.%m", date);
        break;
      }
      case 2: {
        strftime(datebuf, 40, "%d/%m/%Y", date);
      ...
      case 4: {
        strftime(datebuf, 40, "%A, %d.%m.%Y", date);
        break;
      }
      case 5: {
        strftime(datebuf, 40, "%A, %d. %B", date);
        break;
      }
      case 6: {
        strftime(datebuf, 40, "%A, %d. %B %Y", date);
        break;
      }
      case 7: {
        strftime(datebuf, 40, "%A, %d.%m, Week %W", date);
        break;
      }
      case 8: {
        strftime(datebuf, 40, "%A, %d. %B, Week %W", date);
        break;
      }
      case 9: {
        strftime(datebuf, 40, "%A, %d %B %Y, Week %W", date);
        break;
      }
      case 10: {
        strftime(datebuf, 40, "%m/%d/%Y", date);
        break;
      }
      case 11: {
        strftime(datebuf, 40, "%m/%d", date);
        break;
      }
      default:
        break;
  ...
  if ((thcfg->sub("switches")->getBool("usemonthcolors") &&
       rtcfg->getInt("monthcolor") == 0) ||
      rtcfg->getBool("wantshadercolor")) {
...
void drawclock() {
  ...
  clockfontfx.draw(0.0, 0.0, rtcfg->getString("clock"),
                   thcfg->sub("objects")->sub("clock")->getFloat("depth"));
  glPopMatrix();
}
...
void drawdate() {
  ...
  datefontfx.draw(0.0, 0.0, rtcfg->getString("date"),
                  thcfg->sub("objects")->sub("date")->getFloat("depth"));
  glPopMatrix();
}
...
void drawtitles() {
  ...
  if (Ishowmenu() >= 0 && Iem() == 0) {
    drawtitletext(title(Ishowmenu() - 1));
  }

  // emulator listing

  if (Ishowmenu() >= 1 && Iem() != 0) {
    if (thcfg->sub("switches")->getBool("useemutitles")) {
      drawtitletext(emutitle(IeID() - 1));
    } else {
      drawtitletext(emulator(IeID() - 1));
    }
  }
  glPopMatrix();
}
...
void drawromdesc() {
  if (Iem() != 0 &&
    ...
    if (thcfg->sub("objects")->sub("rombox")->getBool("enabled")) {
      ...
      texcol("rombox");
      sf::Texture::bind(
          getTexture(rtcfg->sub("textureids")->sub("settings")->id));
      cuboquad("rombox", "rombox");
      glPopMatrix();
    }
    int gameid =
        rmcfg->sub("Roms")->sub(IeID() - 1)->sub(Ics())->getInt("gameid");
    std::string thistext = " ";
    ...
    rtcfg->sub("objects")
        ->sub("description")
        ->set("width", rominfofontfx.getWidth(thistext));

    if (rtcfg->sub("objects")->sub("description")->getFloat("width") >
        rtcfg->sub("objects")->sub("rombox")->getFloat("width")) {
      int N = thistext.length();
      int M =
          lrint((rtcfg->sub("objects")->sub("description")->getFloat("width") +
                 rominfofontfx.getWidth("WWWW")) /
                rtcfg->sub("objects")->sub("rombox")->getFloat("width")) +
          1;

      rtcfg->set(
          "romboxlinespace",
          -(showH(thcfg->sub("objects")->sub("rombox")->getInt("height") / M)));
      ...
      for (pre = i = 0; i < M; ++i) {
        ...
        if (i != M - 1) at = pre + pos;
        textrot("description", thistext.substr(pre, at - pre), &rominfofontfx,
                3);

        rominfofontfx.draw(
            0.0, 0.0, thistext.substr(pre, at - pre),
            thcfg->sub("objects")->sub("description")->getFloat("depth"));
...
void drawrominfo(std::string info) {
  if (Iem() != 0 &&
      (thcfg->sub("objects")->sub(info)->getBool("enabled") || pick() == 4)) {
    std::string thistext;
    if (compare(info, "romtitle"))
      thistext = prcfg->sub("games")
                     ->sub(rmcfg->sub("Roms")
                               ->sub(IeID() - 1)
                               ->sub(Ics())
                               ->getInt("gameid"))
    ...
    rominfofontfx.draw(0.0, 0.0, thistext,
                       thcfg->sub("objects")->sub(info)->getFloat("depth"));
    glPopMatrix();
  }
}
...
void displayfonts() {
  ...
  drawmusicduration();
  drawmusictitle();
  if (thcfg->sub("objects")->sub("title")->getBool("enabled") || pick() == 4) {
    drawtitles();
  }
  if (thcfg->sub("objects")->sub("clock")->getBool("enabled") || pick() == 4) {
    drawclock();
  }
  if (thcfg->sub("objects")->sub("date")->getBool("enabled") || pick() == 4) {
    drawdate();
  }
  if (thcfg->sub("objects")->sub("myip")->getBool("enabled") || pick() == 4) {
    drawsys("myip", rtcfg->getString("ip"));
  }
  if (thcfg->sub("objects")->sub("uptime")->getBool("enabled") || pick() == 4) {
    drawsys("uptime", rtcfg->getString("uptime"));
  }
  if (thcfg->sub("objects")->sub("totalmem")->getBool("enabled") ||
      pick() == 4) {
    drawsys("totalmem", rtcfg->sub("memusage")->getString("total"));
  }
  if (thcfg->sub("objects")->sub("freemem")->getBool("enabled") ||
      pick() == 4) {
    drawsys("freemem", rtcfg->sub("memusage")->getString("free"));
  }
  if (thcfg->sub("objects")->sub("disk1")->getBool("enabled") || pick() == 4) {
    drawsys("disk1", rtcfg->sub("diskusage")->getString("disk1"));
  }
  if (thcfg->sub("objects")->sub("disk2")->getBool("enabled") || pick() == 4) {
    drawsys("disk2", rtcfg->sub("diskusage")->getString("disk2"));
  }
  if (thcfg->sub("objects")->sub("disk3")->getBool("enabled") || pick() == 4) {
    drawsys("disk3", rtcfg->sub("diskusage")->getString("disk3"));
  }
  if (thcfg->sub("objects")->sub("cpu")->getBool("enabled") || pick() == 4) {
    drawsys("cpu", rtcfg->sub("cpuusage")->getString("cpu"));
  }
  if ((thcfg->sub("objects")->sub("notifier")->getBool("enabled") &&
       rtcfg->getBool("shownotify")) ||
      pick() == 4) {
    drawnotify();
  }
  ...
  if (rtcfg->getBool("editmode") || rtcfg->getBool("searchmode")) {
    drawsearcheditbox();
  }
}
...
void domenucurve() {
  for (int z = 1; z < thcfg->sub("ints")->getInt("menulistmax"); z++) {
    thcfg->sub("objects")
        ->sub(thcfg->sub("objects")->get("menumax1")->id + z)
        ->set("posx", thcfg->sub("ints")->getInt("menucurve") * z);
  }

  for (int z = 1; z < thcfg->sub("ints")->getInt("menulistmin"); z++) {
    thcfg->sub("objects")
        ->sub(thcfg->sub("objects")->get("menumin1")->id + z)
        ->set("posx", -thcfg->sub("ints")->getInt("menucurve") * z);
  }
}
...
void delentry(int menu, int here) {
  //	printf("vor delentry here %i size
...
void moveentry(int to) {
  ...
  if (newmenu < 0)
    newmenu = mecfg->size() - 1;
  else if (newmenu > mecfg->size())
    newmenu = 0;
  printf("newmenu ist %i\n", newmenu);

  printf("newmenusize %i\n", mecfg->sub(newmenu)->sub("entry")->size());
  addentry(newmenu, binary, mecfg->sub(newmenu)->sub("entry")->size() + 1,
           args);
...
void addentries(int menu, std::string path) {
  struct dirent** sdit;
  int num_entries;
  if ((num_entries = scandir(path.c_str(), &sdit, fileselectAll, alphasort)) <
      0) {
  } else {
    for (int j = 0; j < num_entries; j++) {
...
void drawhelpscreen() {
  ...
  int ls = 0;
  std::string start =
      lacfg->getString("press") + " " +
      incfg->sub("key")->sub("startselection")->getString("key") + " ";
  std::string plusorminus = lacfg->getString("press") + " " +
                            incfg->sub("key")->sub("plus")->getString("key") +
                            " " + lacfg->getString("or") + " " +
                            incfg->sub("key")->sub("minus")->getString("key") +
                            " ";
  std::string leftorright = lacfg->getString("press") + " " +
                            incfg->sub("key")->sub("left")->getString("key") +
                            " " + lacfg->getString("or") + " " +
                            incfg->sub("key")->sub("right")->getString("key");
  ...
  switch (rtcfg->getInt("showhelp")) {
    ...
    case 1: {
      helpinfo = start + lacfg->getString("hotkey") + " " +
                 incfg->sub("key")
                     ->sub(rtcfg->sub("helpsel")->getInt("key"))
                     ->getString("desc");
    ...
    case 2: {
      helpinfo =
          start + lacfg->getString("mapbutton") + " " +
          incfg->sub("joypad")->getKey(rtcfg->sub("helpsel")->getInt("joypad"));
    ...
    case 4: {
      helpinfo = start + lacfg->getString("togswitch") + " " +
                 thcfg->sub("switches")
                     ->getKey(rtcfg->sub("helpsel")->getInt("switches"));
    ...
    case 5: {
      helpinfo = start + lacfg->getString("togswitch") + " " +
                 tycfg->sub("globalswitches")
                     ->getKey(rtcfg->sub("helpsel")->getInt("globalswitches"));
      max = tycfg->sub("globalswitches")->size();
      ls = max;
      break;
    }
    case 6: {
      helpinfo = start + lacfg->getString("edemu") + " - " + leftorright + " " +
                 lacfg->getString("changeemu");
    ...
    case 7: {
      helpinfo =
          plusorminus + lacfg->getString("value") + " " +
          thcfg->sub("ints")->getKey(rtcfg->sub("helpsel")->getInt("thint"));
    ...
    case 8: {
      helpinfo = plusorminus + lacfg->getString("value") + " " +
                 tycfg->sub("globalints")
                     ->getKey(rtcfg->sub("helpsel")->getInt("globalints"));
    ...
    case 9: {
      helpinfo = plusorminus + lacfg->getString("value") + " " +
                 thcfg->sub("floats")->getKey(
                     rtcfg->sub("helpsel")->getInt("thfloat"));
    ...
    case 14: {
      helpinfo = thcfg->sub("objects")->getKey(rtcfg->getInt("edobject")) +
                 " - " + leftorright + " " + lacfg->getString("changeobject") +
                 " - " + plusorminus + lacfg->getString("value") + " " +
                 rtcfg->sub("object")->getKey(
                     rtcfg->sub("helpsel")->getInt("objects"));
  ...
  rtcfg->set(
      "helpboxlinespace",
      (int)-(showH(thcfg->sub("objects")->sub("setbox")->getInt("height") /
                   (ls + 3))));
  ...
  glPushMatrix();
  glTranslatef(sb->getFloat("posx") + AAA,
               sb->getFloat("posy") - rtcfg->getInt("helpboxlinespace"),
               sz->getInt("posz"));

  font.setColour(
      veccol4f(co->getInt("R"), co->getInt("G"), co->getInt("B"), 255));
  ...
  for (int k = 0; k < max; k++) {
    ...
    switch (rtcfg->getInt("showhelp")) {
      case 0: {
        helptext = lacfg->sub("helpscreens")->getString(k);
        break;
      }
      case 1: {
        if (k != rtcfg->sub("helpsel")->getInt(rtcfg->getInt("showhelp"))) {
          if (strcmp(incfg->sub("key")->sub(k)->getString("key").c_str(),
                     incfg->sub("key")
                         ->sub(rtcfg->sub("helpsel")->getInt(
                             rtcfg->getInt("showhelp")))
                         ->getString("key")
                         .c_str()) == 0)
            doublemap = 1;
        }
        ...
        if (k == rtcfg->sub("helplist")->getInt("left"))
          glTranslatef(
              rtcfg->sub("objects")->sub("setbox")->getFloat("width") / 3,
              rtcfg->sub("helplist")->getInt("left") *
                  rtcfg->getInt("helpboxlinespace"),
              0);

        else if (k == rtcfg->sub("helplist")->getInt("left") +
                          rtcfg->sub("helplist")->getInt("center"))
          glTranslatef(
              rtcfg->sub("objects")->sub("setbox")->getFloat("width") / 3,
              rtcfg->sub("helplist")->getInt("center") *
                  rtcfg->getInt("helpboxlinespace"),
        ...
        helptext = incfg->sub("key")->sub(k)->getString("desc") + " : " +
                   incfg->sub("key")->sub(k)->getString("key");
        break;
      }
      case 2: {
        helptext = incfg->sub("joypad")->getKey(k) + " : " +
                   itos(incfg->sub("joypad")->getInt(k));
        break;
      }
      case 3: {
        helptext = thcfg->sub("paths")->getKey(k) + " : " +
                   thcfg->sub("paths")->getString(k);
        break;
      }
      case 4: {
        std::string booly;
        if (thcfg->sub("switches")->getBool(k))
          booly = "true";
        else
          booly = "false";
      ...
      case 5: {
        std::string booly;
        if (tycfg->sub("globalswitches")->getBool(k))
          booly = "true";
        else
          booly = "false";
      ...
      case 6: {
        helptext = emcfg->sub(rtcfg->getInt("edemu"))->getKey(k) + " : " +
                   emcfg->sub(rtcfg->getInt("edemu"))->getString(k);
        break;
      }
      case 7: {
        if (k == rtcfg->sub("thintlist")->getInt("left"))
          glTranslatef(
              rtcfg->sub("objects")->sub("setbox")->getFloat("width") / 3,
              rtcfg->sub("thintlist")->getInt("left") *
                  rtcfg->getInt("helpboxlinespace"),
              0);

        else if (k == rtcfg->sub("thintlist")->getInt("left") +
                          rtcfg->sub("thintlist")->getInt("center"))
          glTranslatef(
              rtcfg->sub("objects")->sub("setbox")->getFloat("width") / 3,
              rtcfg->sub("thintlist")->getInt("center") *
                  rtcfg->getInt("helpboxlinespace"),
        ...
        helptext = thcfg->sub("ints")->getKey(k) + " : " +
                   itos(thcfg->sub("ints")->getInt(k));
        break;
      }
      case 8: {
        helptext = tycfg->sub("globalints")->getKey(k) + " : " +
                   itos(tycfg->sub("globalints")->getInt(k));
        break;
      }
      case 9: {
        helptext = thcfg->sub("floats")->getKey(k) + " : " +
                   ftos(thcfg->sub("floats")->getFloat(k));
        break;
      }
      case 10: {
        helptext = thcfg->sub("files")->getKey(k) + " : " +
                   thcfg->sub("files")->getString(k);
        break;
      }
      case 11: {
        helptext = tycfg->sub("strings")->getKey(k) + " : " +
                   tycfg->sub("strings")->getString(k);
        break;
      }
      case 12: {
        helptext = tycfg->sub("paths")->getKey(k) + " : " +
                   tycfg->sub("paths")->getString(k);
        break;
      }
      case 13: {
        helptext = getFile(oncfg->sub("themes")->getString(k));
        break;
      }
      case 14: {
        std::string objhelp = rtcfg->sub("object")->getString(k) + " : ";
        TyphonConfig* val =
            thcfg->sub("objects")->sub(rtcfg->getInt("edobject"));
        std::string keyname = rtcfg->sub("object")->getKey(k);
        if (val->exists(keyname)) {
          if (k == 0 || k == 15 || k == 16) {
            std::string booly;
            if (val->getBool(keyname))
              booly = "true";
            else
              booly = "false";
          ...
          helptext = objhelp + lacfg->getString("notavailable");
          break;
        }
      }
    ...
    glPushMatrix();
    if (k == rtcfg->sub("helpsel")->getInt(rtcfg->getInt("showhelp")))
      font.setColour(veccol4f(rtcfg->getInt("tempselr"),
                              rtcfg->getInt("tempselg"),
                              rtcfg->getInt("tempselb"), 255));
    else if (doublemap)
      font.setColour(veccol4f(255, 1, 1, 255));
    else
      textcol("title", &font, 0);
