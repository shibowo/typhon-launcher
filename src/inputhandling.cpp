...
void startprog() {
  togentryname(1);
  if ((Iem() != 0 && !expathcheck(emulator(IeID() - 1).c_str())) ||
      (Iem() == 0 && !expathcheck(mentry(Ishowmenu() - 1, Ics()).c_str()))) {
    if (tycfg->sub("globalswitches")->getBool("stopsoundonlaunch")) {
      if (thcfg->sub("switches")->getBool("playsound")) {
        rtcfg->setBool("soundwason", 1);
        stopsound();
      }
      if (thcfg->sub("switches")->getBool("playmusic")) {
        rtcfg->setBool("musicwason", 1);
        stopmusic();
      }
    }

    if (thcfg->sub("switches")->getBool("stopvideoonlaunch") &&
        thcfg->sub("objects")->sub("video")->getInt("drawhere") != 0)
      rtcfg->setBool("videowason", 1);
    stopvideo();

    if (tycfg->sub("globalswitches")->getBool("stopjoypadonlaunch") &&
        rtcfg->getBool("usejoypad")) {
      rtcfg->setBool("joypadwason", 1);
      ;
      rtcfg->setBool("usejoypad", 0);
    }

    if (tycfg->sub("globalswitches")->getBool("stopshaderonlaunch")) {
      if (useShader()) rtcfg->setBool("shaderwas", useShader());
      thcfg->sub("shader")->setBool("useshader", 0);
    }

    if (Iem() != 0) {
      std::string romargs = emuargs(IeID() - 1) + " \"" + rompath(IeID() - 1) +
                            "/" + romname(IeID() - 1) + "\"";
...
#endif  // DEBUG
      if (tycfg->sub("globalswitches")->getBool("autoscreenshot")) {
        startscrotting(getPrefix(romname(IeID() - 1)),
                       rompicturespath(IeID() - 1));
      }
    } else {
      systemstart(mentry(Ishowmenu() - 1, Ics()), margs(Ishowmenu() - 1, Ics()),
                  0);
#ifdef DEBUG
      printf("INFO    - commandline for program is %s %s\n",
             mentry(Ishowmenu() - 1, Ics()).c_str(),
             margs(Ishowmenu() - 1, Ics()).c_str());
#endif  // DEBUG
      if (tycfg->sub("globalswitches")->getBool("autoscreenshot")) {
        startscrotting(mentry(Ishowmenu() - 1, Ics()),
                       rtcfg->sub("scanned")->getString("previewpath"));
      }
    }
    ...
    printf("program doesn't exist - doing nothing\n");
#endif  // DEBUG
  togentryname(0);
}
...
void toggleglobalswitch() {
  ...
  tycfg->sub("globalswitches")
      ->setBool(curswitch, !tycfg->sub("globalswitches")->getBool(curswitch));

  if (strcmp(tycfg->sub("globalswitches")->getKey(curswitch).c_str(),
             "hidemouse") == 0) {
...
void togglethswitch() {
  ...
  thcfg->sub("switches")
      ->setBool(curswitch, !thcfg->sub("switches")->getBool(curswitch));

  //	if (strcmp(thcfg->sub("switches")->getKey(curswitch).c_str(),
  //"hidemouse")==0)
  //{window.ShowMouseCursor(!thcfg->sub("switches")->getBool(curswitch));}
}

int helpid(std::string help) {
  return lacfg->sub("helpscreens")->get(help)->id;
}

void selchanged() {
  if (!rtcfg->getBool("showhelpscreen") /*&& pick()==0*/ &&
      !rtcfg->getBool("menuisscrolling"))
    rtcfg->setBool("selchanged", 1);
}

void sethelp(int a) {
  if (rtcfg->getInt("showhelp") == a) {
    rtcfg->setBool("showhelpscreen", 0);
    rtcfg->set("showhelp", helpid("helpscreenlist"));
  } else
...
void changeobjvalue(int ch) {
  ...
  if (val->exists(keyname)) {
    ...
    {
      ((TConfig<float>*)val->get(keyname))
          ->operator+(
              ch * (float)tycfg->sub("globalints")->getInt("changevaluespeed"));
    } else  // ints
    {
      ((TConfig<int>*)val->get(keyname))
          ->operator+(ch *
                      tycfg->sub("globalints")->getInt("changevaluespeed"));
      if (k == 1 || k == 2) {
        rtcfg->sub("objects")->sub(object)->set(
            "width", showW(thcfg->sub("objects")
                               ->sub(rtcfg->getInt("edobject"))
                               ->getInt("width")));
        rtcfg->sub("objects")->sub(object)->set(
            "height", showH(thcfg->sub("objects")
                                ->sub(rtcfg->getInt("edobject"))
                                ->getInt("height")));
      }
      if (k == 5 || k == 6) {
        rtcfg->sub("objects")->sub(object)->set(
            "posx", showX(thcfg->sub("objects")
                              ->sub(rtcfg->getInt("edobject"))
                              ->getInt("posx")));
        rtcfg->sub("objects")->sub(object)->set(
            "posy", showY(thcfg->sub("objects")
                              ->sub(rtcfg->getInt("edobject"))
                              ->getInt("posy")));
      }
    }
...
void startselection() {
  if (rtcfg->getBool("showhelpscreen")) {
    switch (rtcfg->getInt("showhelp")) {
      case 0: {
        sethelp(rtcfg->sub("helpsel")->getInt("helplist"));
        break;
      }
      case 1: {
        rtcfg->setBool("editkeys", 1);
        break;
      }
      case 2: {
        rtcfg->setBool("waitforjoypad", 1);
        break;
      }
      case 3: {
        rtcfg->set("inputchanged", 2);
        editorinput(thcfg->sub("paths")->getString(
                        rtcfg->sub("helpsel")->getInt("paths")),
                    4);
        break;
      }
      case 4: {
      ...
      case 6: {
        rtcfg->set("inputchanged", 2);
        editorinput(emcfg->sub(rtcfg->getInt("edemu"))
                        ->getString(rtcfg->sub("helpsel")->getInt("emu")),
                    6);
        break;
      }
      case 11: {
        rtcfg->set("inputchanged", 2);
        editorinput(tycfg->sub("strings")->getString(
                        rtcfg->sub("helpsel")->getInt("stringedit")),
                    9);
        break;
      }
      case 12: {
        rtcfg->set("inputchanged", 2);
        editorinput(tycfg->sub("paths")->getString(
                        rtcfg->sub("helpsel")->getInt("globalpaths")),
                    5);
        break;
      }
      case 13: {
    ...
    if (rtcfg->getInt("edmode") == 1) {
      ...
      if (Ishowmenu() >= 0 && Iem() == 0) {
        rtcfg->set("inputchanged", 2);
        editorinput(title(Ishowmenu() - 1), 7);
      }
      if (Ishowmenu() >= 1 && !Bvidmenu() && !Bp3tmenu() && !Bthememenu() &&
          Iem() != 0 && thcfg->sub("switches")->getBool("useemutitles")) {
        rtcfg->set("inputchanged", 2);
        editorinput(emutitle(IeID() - 1), 8);
      }
    }
  }
    ...
    if (Ishowmenu() == 0) {
      ...
      if (!Bvidmenu() && !Bp3tmenu() && !Bthememenu()) {
        startprog();
      }
      if (Iem() == 0 && Bvidmenu()) {
        startplayvideo();
...
void restartmedia() {
  if (rtcfg->getBool("programwasstarted")) {
    if (rtcfg->getBool("soundwason") || rtcfg->getBool("musicwason")) {
      initaudio();
    };
    if (rtcfg->getBool("soundwason")) {
      togglesound();
      rtcfg->setBool("soundwason", 0);
    };
    if (rtcfg->getBool("musicwason")) {
      togglemusic();
      rtcfg->setBool("musicwason", 0);
    };
    if (rtcfg->getBool("videowason")) {
      startplayvideo();
      rtcfg->setBool("videowason", 0);
    };
    if (rtcfg->getBool("joypadwason")) {
      rtcfg->setBool("usejoypad", 1);
      rtcfg->setBool("joypadwason", 0);
    };
    if (rtcfg->getBool("shaderwas")) {
      thcfg->sub("shader")->setBool("useshader", rtcfg->getBool("shaderwas"));
      loadshadertex();
      rtcfg->setBool("shaderwas", 0);
    };
...
void edchangevalue(int cone) {
  if (rtcfg->getBool("menueditmode")) {
    if (Ics() + (cone * -1) < 0 ||
        Ics() + (cone * -1) >=
            mecfg->sub(Ishowmenu() - 1)->sub("entry")->size() - 1) {
    } else {
      mecfg->sub(Ishowmenu() - 1)
          ->sub("entry")
          ->move(Ics(), Ics() + (cone * -1));
      if ((cone * -1) > 0)
        dokeydown();
    ...
    ((TConfig<int>*)tycfg->sub("globalints")->get("changevaluespeed"))
        ->operator+(cone);
    shownote(lacfg->getString("valuespeed") +
             itos(tycfg->sub("globalints")->getInt("changevaluespeed")));
  }
}
...
void domenuchange(int here) {
  if (rtcfg->getBool("showhelpscreen")) {
    if (rtcfg->getInt("showhelp") == helpid("themefiles")) {
      ...
      if (rtcfg->sub("helpsel")->getInt("datafile") ==
          thcfg->sub("files")->get("fontfile")->id)
        switchfont(ch);
      if (rtcfg->sub("helpsel")->getInt("datafile") ==
          thcfg->sub("files")->get("musicfile")->id)
        switchmusic(ch);
      if (rtcfg->sub("helpsel")->getInt("datafile") ==
          thcfg->sub("files")->get("soundfile")->id)
        switchsfx(ch);
#ifdef WITHMMD
      if (rtcfg->sub("helpsel")->getInt("datafile") ==
          thcfg->sub("files")->get("mmdmodelfile")->id)
        updatemodel(ch);
      if (rtcfg->sub("helpsel")->getInt("datafile") ==
          thcfg->sub("files")->get("mmdmotionfile")->id)
        updatemotion(ch);
#endif  // WITHMMD
    } else {
    }
    ...
    if (thcfg->sub("switches")->getBool("randomwallpaper") &&
        !thcfg->sub("switches")->getBool("usemenuwallpaper")) {
      rtcfg->set("currentwp", rand() % rtcfg->sub("wallpapers")->size());
      sTex("wallpaper", rtcfg->sub("wallpapers")
                            ->sub(rtcfg->getInt("currentwp"))
                            ->getString("file"));
      updatewallpaper(
          thcfg->sub("textures")->getStringP("wallpaper"),
          getTexture(rtcfg->sub("textureids")->sub("wallpaper")->id));
    }
    ...
    if (thcfg->sub("switches")->getBool("usemenuwallpaper")) {
      rtcfg->set("blendwall", (float)1);
      rtcfg->setBool("wallscroll", here - 1);
    }
...
void dokeyleft() {
  ...
  if (rtcfg->getBool("showhelpscreen")) {
    if (rtcfg->getInt("showhelp") == helpid("hotkeylist")) {
      playsfx();
      ((TConfig<int>*)rtcfg->sub("helpsel")->get("key"))
          ->operator-(incfg->sub("key")->size() / 3);
      if (rtcfg->sub("helpsel")->getInt("key") < 0)
        rtcfg->sub("helpsel")->setC("key", incfg->sub("key")->size() - 1);
      ...
      ((TConfig<int>*)rtcfg->sub("helpsel")->get("thint"))
          ->operator-(thcfg->sub("ints")->size() / 3);
      if (rtcfg->sub("helpsel")->getInt("thint") < 0)
        rtcfg->sub("helpsel")->setC("thint", thcfg->sub("ints")->size() - 1);
    } else if (rtcfg->getInt("showhelp") == helpid("objects")) {
      ((TConfig<int>*)rtcfg->get("edobject"))->operator-(1);
      if (rtcfg->getInt("edobject") < 0)
        rtcfg->set("edobject", thcfg->sub("objects")->size() - 1);
    }
  } else {
    domenuchange(1);
  }
...
void dokeyright() {
  ...
  if (rtcfg->getBool("showhelpscreen")) {
    if (rtcfg->getInt("showhelp") == helpid("hotkeylist")) {
      ((TConfig<int>*)rtcfg->sub("helpsel")->get("key"))
          ->operator+(incfg->sub("key")->size() / 3);
      if (rtcfg->sub("helpsel")->getInt("key") > incfg->sub("key")->size() - 1)
        rtcfg->sub("helpsel")->setC("key", 0);
    } else if (rtcfg->getInt("showhelp") == helpid("emulatorsettings")) {
      ((TConfig<int>*)rtcfg->get("edemu"))->operator+(1);
      ...
      ((TConfig<int>*)rtcfg->sub("helpsel")->get("thint"))
          ->operator+(thcfg->sub("ints")->size() / 3);
      if (rtcfg->sub("helpsel")->getInt("thint") >
          thcfg->sub("ints")->size() - 1)
        rtcfg->sub("helpsel")->setC("thint", 0);
    } else if (rtcfg->getInt("showhelp") == helpid("objects")) {
      ((TConfig<int>*)rtcfg->get("edobject"))->operator+(1);
      if (rtcfg->getInt("edobject") > thcfg->sub("objects")->size() - 1)
        rtcfg->set("edobject", 0);
    }
  } else {
    domenuchange(2);
  }
...
void chthint(int ch) {
  ...
  if (thcfg->sub("ints")->getInt(curthint) > max) {
    thcfg->sub("ints")->set(curthint, min);
  } else if (thcfg->sub("ints")->getInt(curthint) < min) {
    thcfg->sub("ints")->set(curthint, max);
  }

  if (strcmp(thcfg->sub("ints")->getKey(curthint).c_str(), "dateformat") == 0) {
    rtcfg->setBool("havethedate", 0);
  } else if (strcmp(thcfg->sub("ints")->getKey(curthint).c_str(),
                    "menucurve") == 0) {
    domenucurve();
  } else if (strcmp(thcfg->sub("ints")->getKey(curthint).c_str(),
                    "videovolume") == 0) {
    setvideovol(thcfg->sub("ints")->getInt("videovolume"));
                    ...
                    "musicvolume") == 0) {
    musicvol(thcfg->sub("ints")->getInt("musicvolume"));
  }
}
...
void chglobint(int ch) {
  ...
  if (gi->getInt(curglobint) > max) {
    gi->set(curglobint, min);
  } else if (gi->getInt(curglobint) < min) {
    gi->set(curglobint, max);
  }
  /*
                   if (strcmp(gi->getKey(curglobint).c_str(), "width")==0 ||
                           strcmp(gi->getKey(curglobint).c_str(), "height")==0)
                          {resizescreen(gi->getInt("width"),
     gi->getInt("height"));}
  */
  if (strcmp(gi->getKey(curglobint).c_str(), "windowposx") == 0) {
    windowpos(gi->getInt("windowposx"), gi->getInt("windowposy"));
  } else if (strcmp(gi->getKey(curglobint).c_str(), "windowposy") == 0) {
    windowpos(gi->getInt("windowposx"), gi->getInt("windowposy"));
  } else if (strcmp(gi->getKey(curglobint).c_str(), "thememenu") == 0) {
    if (Bthememenu()) setcs(0);
  } else if (strcmp(gi->getKey(curglobint).c_str(), "p3tmenu") == 0) {
    if (Bp3tmenu()) setcs(0);
  }
}
...
void chthfloat(float ch) {
  ...
  ((TConfig<float>*)thcfg->sub("floats")->get(curthfloat))
      ->operator+(ch * (max / 100));
  if (thcfg->sub("floats")->getFloat(curthfloat) > max) {
    thcfg->sub("floats")->set(curthfloat, min);
  } else if (thcfg->sub("floats")->getFloat(curthfloat) < min) {
    thcfg->sub("floats")->set(curthfloat, max);
  }

  if (strcmp(thcfg->sub("floats")->getKey(curthfloat).c_str(),
             "menuiconspace") == 0) {
    rtcfg->set("showmiconspace",
               thcfg->sub("floats")->getFloat("menuiconspace") * icwidth);
  } else if (strcmp(thcfg->sub("floats")->getKey(curthfloat).c_str(),
...
void dokeychangeval(bool up) {
  ...
  if (rtcfg->getBool("menueditmode")) {
    if (up) {
      addentry(
          Ishowmenu() - 1,
          "startme" + itos(mecfg->sub(Ishowmenu() - 1)->sub("entry")->size()),
          Ics(), "");
    } else {
    ...
    if (rtcfg->getBool("showhelpscreen")) {
      if (rtcfg->getInt("showhelp") == helpid("themeints")) {
        chthint(ch);
      }
      if (rtcfg->getInt("showhelp") == helpid("globalints")) {
        chglobint(ch);
      }
      if (rtcfg->getInt("showhelp") == helpid("themefloats")) {
        chthfloat((float)ch);
      }
      if (rtcfg->getInt("showhelp") == helpid("objects")) {
        changeobjvalue(ch);
      }
    } else if (rtcfg->getBool("choosemicon") && !rtcfg->getBool("editkeys") &&
               !rtcfg->getBool("choosewp")) {
      changemenuicon(up);
    } else if (rtcfg->getBool("choosewp") && !rtcfg->getBool("editkeys") &&
               !rtcfg->getBool("choosemicon")) {
      changewallpaper(up);
      loadshadertex();
    }
  }
...
void menupageup() {
  ...
  if (Bvidmenu()) {
    ...
    if (Ics() <= tycfg->sub("globalints")->getInt("changevaluespeed"))
      dokeyup();
    else {
      setcs((Ics() - tycfg->sub("globalints")->getInt("changevaluespeed")));
      selchanged();
    }
  }
}
  ...
  if (!rtcfg->getBool("showhelpscreen")) {
    ...
    switch (rtcfg->getInt("showhelp")) {
      case 0: {
        ...
        if (hs->getInt("helplist") < 0)
          hs->setC("helplist", lacfg->sub("helpscreens")->size() - 1);
        break;
      }
      case 1: {
        ((TConfig<int>*)hs->get("key"))->operator-(1);
        if (hs->getInt("key") < 0)
          hs->setC("key", incfg->sub("key")->size() - 1);
        break;
      }
      case 2: {
        ((TConfig<int>*)hs->get("joypad"))->operator-(1);
        if (hs->getInt("joypad") < 0)
          hs->setC("joypad", incfg->sub("joypad")->size() - 1);
        break;
      }
      case 3: {
        ((TConfig<int>*)hs->get("paths"))->operator-(1);
        if (hs->getInt("paths") < 0)
          hs->setC("paths", thcfg->sub("paths")->size() - 1);
        break;
      }
      case 4: {
        ((TConfig<int>*)hs->get("switches"))->operator-(1);
        if (hs->getInt("switches") < 0)
          hs->setC("switches", thcfg->sub("switches")->size() - 1);
        break;
      }
      case 5: {
        ((TConfig<int>*)hs->get("globalswitches"))->operator-(1);
        if (hs->getInt("globalswitches") < 0)
          hs->setC("globalswitches", tycfg->sub("globalswitches")->size() - 1);
        break;
      }
      case 6: {
        ((TConfig<int>*)hs->get("emu"))->operator-(1);
        if (hs->getInt("emu") < 0)
          hs->setC("emu", emcfg->sub(rtcfg->getInt("edemu"))->size() - 1);
        break;
      }
      case 7: {
        ((TConfig<int>*)hs->get("thint"))->operator-(1);
        if (hs->getInt("thint") < 0)
          hs->setC("thint", thcfg->sub("ints")->size() - 1);
        break;
      }
      case 8: {
        ((TConfig<int>*)hs->get("globalints"))->operator-(1);
        if (hs->getInt("globalints") < 0)
          hs->setC("globalints", tycfg->sub("globalints")->size() - 1);
        break;
      }
      case 9: {
        ((TConfig<int>*)hs->get("thfloat"))->operator-(1);
        if (hs->getInt("thfloat") < 0)
          hs->setC("thfloat", thcfg->sub("floats")->size() - 1);
        break;
      }
      case 10: {
        ((TConfig<int>*)hs->get("datafile"))->operator-(1);
        if (hs->getInt("datafile") < 0)
          hs->setC("datafile", thcfg->sub("files")->size() - 1);
        break;
      }
      case 11: {
        ((TConfig<int>*)hs->get("stringedit"))->operator-(1);
        if (hs->getInt("stringedit") < 0)
          hs->setC("stringedit", tycfg->sub("strings")->size() - 1);
        break;
      }
      case 12: {
        ((TConfig<int>*)hs->get("globalpaths"))->operator-(1);
        if (hs->getInt("globalpaths") < 0)
          hs->setC("globalpaths", tycfg->sub("paths")->size() - 1);
        break;
      }
      case 13: {
        ((TConfig<int>*)hs->get("online"))->operator-(1);
        if (hs->getInt("online") < 0)
          hs->setC("online", oncfg->sub("themes")->size() - 1);
        break;
      }
      case 14: {
        ((TConfig<int>*)hs->get("objects"))->operator-(1);
        if (hs->getInt("objects") < 0)
          hs->setC("objects", rtcfg->sub("object")->size() - 1);
        break;
      }
      default:
        break;
...
void dokeydown() {
  ...
  if (!rtcfg->getBool("showhelpscreen")) {
    ...
    switch (rtcfg->getInt("showhelp")) {
      case 0: {
        ((TConfig<int>*)hs->get("helplist"))->operator+(1);
        if (hs->getInt("helplist") > lacfg->sub("helpscreens")->size() - 1)
          hs->setC("helplist", 0);
        break;
      }
      case 1: {
        ((TConfig<int>*)hs->get("key"))->operator+(1);
        if (hs->getInt("key") > incfg->sub("key")->size() - 1)
          hs->setC("key", 0);
        break;
      }
      case 2: {
        ((TConfig<int>*)hs->get("joypad"))->operator+(1);
        if (hs->getInt("joypad") > incfg->sub("joypad")->size() - 1)
          hs->setC("joypad", 0);
        break;
      }
      case 3: {
        ((TConfig<int>*)hs->get("paths"))->operator+(1);
        if (hs->getInt("paths") > thcfg->sub("paths")->size() - 1)
          hs->setC("paths", 0);
        break;
      }
      case 4: {
        ((TConfig<int>*)hs->get("switches"))->operator+(1);
        if (hs->getInt("switches") > thcfg->sub("switches")->size() - 1)
          hs->setC("switches", 0);
        break;
      }
      case 5: {
        ((TConfig<int>*)hs->get("globalswitches"))->operator+(1);
        if (hs->getInt("globalswitches") >
            tycfg->sub("globalswitches")->size() - 1)
          hs->setC("globalswitches", 0);
        break;
      }
      case 6: {
        ((TConfig<int>*)hs->get("emu"))->operator+(1);
        if (hs->getInt("emu") > emcfg->sub(rtcfg->getInt("edemu"))->size() - 1)
          hs->setC("emu", 0);
        break;
      }
      case 7: {
        ((TConfig<int>*)hs->get("thint"))->operator+(1);
        if (hs->getInt("thint") > thcfg->sub("ints")->size() - 1)
          hs->setC("thint", 0);
        break;
      }
      case 8: {
        ((TConfig<int>*)hs->get("globalints"))->operator+(1);
        if (hs->getInt("globalints") > tycfg->sub("globalints")->size() - 1)
          hs->setC("globalints", 0);
        break;
      }
      case 9: {
        ((TConfig<int>*)hs->get("thfloat"))->operator+(1);
        if (hs->getInt("thfloat") > thcfg->sub("floats")->size() - 1)
          hs->setC("thfloat", 0);
        break;
      }
      case 10: {
        ((TConfig<int>*)hs->get("datafile"))->operator+(1);
        if (hs->getInt("datafile") > thcfg->sub("files")->size() - 1)
          hs->setC("datafile", 0);
        break;
      }
      case 11: {
        ((TConfig<int>*)hs->get("stringedit"))->operator+(1);
        if (hs->getInt("stringedit") > tycfg->sub("strings")->size() - 1)
          hs->setC("stringedit", 0);
        break;
      }
      case 12: {
        ((TConfig<int>*)hs->get("globalpaths"))->operator+(1);
        if (hs->getInt("globalpaths") > tycfg->sub("paths")->size() - 1)
          hs->setC("globalpaths", 0);
        break;
      }
      case 13: {
        ((TConfig<int>*)hs->get("online"))->operator+(1);
        if (hs->getInt("online") > oncfg->sub("themes")->size() - 1)
          hs->setC("online", 0);
        break;
      }
      case 14: {
        ((TConfig<int>*)hs->get("objects"))->operator+(1);
        if (hs->getInt("objects") > rtcfg->sub("object")->size() - 1)
          hs->setC("objects", 0);
        break;
      }
      default:
...
void pressescape() {
  if (rtcfg->getBool("showhelpscreen")) {
    rtcfg->setBool("showhelpscreen", 0);
    rtcfg->set("showhelp", helpid("helpscreenlist"));
  } else if (thcfg->sub("objects")->sub("video")->getInt("drawhere") == 3)
    stopvideo();
  else
    window.close();
}
...
bool checkkey(std::string Key, std::string comp) {
  if (strcasecmp(incfg->sub("key")->sub(Key)->getString("key").c_str(),
                 comp.c_str()) == 0)
    return 1;
  else
    return 0;
...
void toggleedit() {
  if (rtcfg->getBool("showhelpscreen"))
    ...
    switch (rtcfg->getInt("edmode")) {
      case 0: {
        shownote(lacfg->getString("editmode") + " " +
                 lacfg->getString("disabled"));
        break;
      }
      case 1: {
        shownote(lacfg->getString("editmode") + " " +
                 lacfg->getString("menuentries"));
        break;
      }
      case 2: {
        shownote(lacfg->getString("editmode") + " " +
                 lacfg->getString("cmdargs"));
        break;
      }
      case 3: {
        shownote(lacfg->getString("editmode") + " " +
                 lacfg->getString("title"));
        break;
      }
      default:
        break;
...
void input() {
  ...
  while (window.pollEvent(event)) {
    if ((rtcfg->getBool("searchmode") || rtcfg->getBool("editmode")) &&
      ...
      if (rtcfg->getBool("searchmode")) {
        searchinput();
        break;
      }

      if (rtcfg->getBool("exporting")) {
      ...
      if (rtcfg->getBool("editmode") &&
          ((Ishowmenu() >= 0) &&
           (Iem() == 0 || (Iem() != 0 && rtcfg->getInt("edmode") == 3))) &&
          !Bvidmenu() && !Bp3tmenu() && !Bthememenu()) {
        startselection();
        break;
      }
    }
    ...
    if ((!rtcfg->getBool("searchmode") && !rtcfg->getBool("editmode") &&
         rtcfg->getInt("inputchanged") == 0) &&
        (event.type == event.KeyPressed)) {
      ekey = event.key.code;

      if (rtcfg->getBool("editkeys")) {
        std::string edkeystring =
            incfg->sub("key")->getKey(rtcfg->sub("helpsel")->getInt("key"));
        ...
        if (thcfg->sub("objects")->sub("video")->getInt("drawhere") != 3) {
          ...
          if (ekey == ksf("changeicon")) {
            ...
            if (rtcfg->getBool("choosemicon")) {
              rtcfg->setBool("choosewp", 0);
              shownote(lacfg->getString("changemenuiconon"));
            } else {
              shownote(lacfg->getString("changemenuiconoff"));
            }
          }
          if (ekey == ksf("changebg")) {
            rtcfg->setBool("choosewp", !rtcfg->getBool("choosewp"));
            if (rtcfg->getBool("choosewp")) {
              rtcfg->setBool("choosemicon", 0);
              shownote(lacfg->getString("changewpon"));
            } else {
              shownote(lacfg->getString("changewpoff"));
            }
          }
          ...
          if (ekey == ksf("togglesetpicking")) {
            setpicktog();
          }
          if (ekey == ksf("togglecolorpicking")) {
            colorpicktog();
          }
          if (ekey == ksf("rotationpicking")) {
            changerotpick();
          }
          if (ekey == ksf("toggleedit")) {
            toggleedit();
          }
          if (ekey == ksf("searchmode")) {
            rtcfg->set("inputchanged", 3);
          }
          //					if (ekey==ksf(Keyreloadxml))
          //{if(thcfg->sub("objects")->sub("slide1")->getBool("enabled")){readxmlprogram();}}
          //					if
          //(ekey==ksf(Keytoggleslideshow))	{toggleslideshow();} 					if
          //(ekey==ksf("rss"))
          //thcfg->sub("objects")->sub("rss")->getBool("enabled"));if(thcfg->sub("objects")->sub("rss")->getBool("enabled")
          //&&!rtcfg->getBool("rssinitialized")){initrss();readrss();}};
          if (ekey == ksf("help")) {
            rtcfg->setBool("showhelpscreen", 1);
            sethelp(1);
          }
          if (ekey == ksf("switchhelpscreen")) {
            rtcfg->setBool("showhelpscreen", !rtcfg->getBool("showhelpscreen"));
          }

          if (ekey == ksf("savecfgs")) {
            saveglobalcfgs();
          }
          if (ekey == ksf("savetheme")) {
            savetheme();
          }

          if (ekey == ksf("updatemenu")) {
            parsedesktopfiles();
            savemenuxmlcfg();
          }
          if (ekey == ksf("favourites")) {
            addtofavourites();
            savemenuxmlcfg();
          }
          ...
          if (ekey == ksf("decchangevalue")) {
            playsfx();
            edchangevalue(-1);
          }
          if (ekey == ksf("incchangevalue")) {
            playsfx();
            edchangevalue(1);
          }
          if (ekey == ksf("fastdown")) {
            menupagedown();
          }
          if (ekey == ksf("fastup")) {
            menupageup();
          }
          if (ekey == ksf("toggleemu")) {
            toggleemumode();
          }
          if (ekey == ksf("togglevideomenu")) {
            togglevideomenu();
          }
          ...
          if (ekey == ksf("changeshader")) {
            ((TConfig<int>*)thcfg->sub("shader")->get("usebackshader"))
                ->operator+(1);
            if (useBackShader() >= shcfg->sub("shaders")->size()) {
              thcfg->sub("shader")->set("usebackshader", 0);
            }
            loadshadertex();
            shownote(lacfg->getString("changeshader") + " " +
                     getPrefix(shcfg->sub("shaders")
                                   ->sub(useBackShader())
                                   ->getString("name")) +
                     " " + lacfg->getString("createdby") + " " +
                     shcfg->sub("shaders")
                         ->sub(useBackShader())
                         ->getString("author"));
          }
        ...
        if (ekey == ksf("movewindow"))
          movewindow(tycfg->sub("globalswitches")->getBool("movewindowup"));
        if (ekey == ksf("togglevideo")) switchplayvideohere();
        if (ekey == ksf("togglepicking")) mousepicktog();
        if (ekey == ksf("videovolume"))
          videovol(tycfg->sub("globalints")->getInt("changevaluespeed"));
        if (ekey == ksf("menueditmode")) {
          rtcfg->setBool("menueditmode", !rtcfg->getBool("menueditmode"));
          if (rtcfg->getBool("menueditmode")) {
            shownote(lacfg->getString("menueditmode") + " " +
                     lacfg->getString("enabled"));
          } else {
            shownote(lacfg->getString("menueditmode") + " " +
                     lacfg->getString("disabled"));
          }
    ...
    if ((!rtcfg->getBool("searchmode") && !rtcfg->getBool("editmode")) &&
      ...
      if (!stillpressed) {
        if (ekey == ksf("up") || ekey == ksf("down") ||
            ekey == ksf("fastdown") || ekey == ksf("fastup")) {
          rtcfg->set("tempselr", thcfg->sub("objects")
                                     ->sub("menu")
                                     ->sub("menuselfont")
                                     ->getInt("R"));
          rtcfg->set("tempselg", thcfg->sub("objects")
                                     ->sub("menu")
                                     ->sub("menuselfont")
                                     ->getInt("G"));
          rtcfg->set("tempselb", thcfg->sub("objects")
                                     ->sub("menu")
                                     ->sub("menuselfont")
                                     ->getInt("B"));
        ...
        if (ekey == ksf("up") || ekey == ksf("down") || ekey == ksf("right") ||
            ekey == ksf("left") || ekey == ksf("toggleemu") ||
            ekey == ksf("fastdown") || ekey == ksf("fastup") ||
            ekey == ksf("minus") || ekey == ksf("plus")) {
          selchanged();
          if (thcfg->sub("objects")->sub("menuicon")->getBool("enabled") &&
              thcfg->sub("ints")->getInt("menuicondockstyle") == 1) {
            rtcfg->setBool("undock", 1);
          }
        }
      }
    }
      ...
      if (rtcfg->getBool("waitforjoypad")) {
        ...
        while (!done) {
          if (event.JoystickButtonPressed) {
            incfg->sub("joypad")->set(rtcfg->sub("helpsel")->getInt("joypad"),
        ...
        if (thcfg->sub("objects")->sub("video")->getInt("drawhere") != 3) {
          ...
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("dechelpscreen")))
            sethelp(0);
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("inchelpscreen")))
            rtcfg->setBool("showhelpscreen", !rtcfg->getBool("showhelpscreen"));
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("nexttyphontheme")))
            nexttyphontheme();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("switchplayvideohere")))
            switchplayvideohere();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("menupageup")))
            menupageup();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("menupagedown")))
            menupagedown();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("togglevideomenu")))
            togglevideomenu();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("toggleemumode")))
            toggleemumode();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("escape")))
            pressescape();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("startselection")))
            startselection();
          if (sf::Joystick::isButtonPressed(jodev,
                                            incfg->sub("joypad")->getInt("up")))
            dokeyup();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("right")))
            dokeyright();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("down")))
            dokeydown();
          if (sf::Joystick::isButtonPressed(
                  jodev, incfg->sub("joypad")->getInt("left")))
            dokeyleft();
        }
      }
    }
    ...
    if (event.type == event.JoystickMoved && rtcfg->getBool("usejoypad")) {
      if (thcfg->sub("objects")->sub("video")->getInt("drawhere") != 3) {
        if (event.joystickMove.axis == sf::Joystick::X) {
          if (event.joystickMove.position >
              tycfg->sub("globalints")->getInt("joypaddeadzone"))
            dokeyright();
          else if (event.joystickMove.position <
                   -tycfg->sub("globalints")->getInt("joypaddeadzone"))
            dokeyleft();
    ...
    if (event.type == event.MouseButtonPressed) {
      if (thcfg->sub("objects")->sub("video")->getInt("drawhere") == 3 &&
        ...
        switch (event.mouseButton.button) {
          case sf::Mouse::Left: {
            rtcfg->setBool("clickedleft", 1);
            break;
          }
          case sf::Mouse::Right: {
            rtcfg->setBool("clickedright", 1);
            break;
          }
          default:
            break;
        ...
        switch (event.mouseButton.button) {
          ...
          case sf::Mouse::Middle: {
            if (pick() == 1) {
              ...
              if (rtcfg->getBool("rotatez"))
                shownote(lacfg->getString("zrot"));
              else
                shownote(lacfg->getString("xyrot"));
              break;
            }
            if (pick() == 2) {
              mouseblockobjects();
    ...
    if (event.type == event.MouseWheelMoved) {
      if (thcfg->sub("objects")->sub("video")->getInt("drawhere") != 3) {
        if (event.mouseWheel.delta == 1) {
          ...
          if (pick() == 1) {
            mousedepthobjects(
                -(float)tycfg->sub("globalints")->getInt("changevaluespeed"));
            break;
          }
    ...
    //		if(event.type==event.Resized){resizescreen(event.size.width,
    //event.size.height);}

    if (event.type == event.MouseMoved) {
      if (pick() == 1 || pick() == 2 || pick() == 3 || pick() == 4 ||
          (shcfg->sub("shaders")->sub(useBackShader())->getBool("mouse") &&
           useShader())) {
