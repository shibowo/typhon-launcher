...
void scrollrss(int rssspeed) {
  ((TConfig<float>*)rtcfg->sub("objects")->sub("rss")->get("posx"))
      ->operator-(rssspeed);
  if (rtcfg->sub("objects")->sub("rss")->getFloat("posx") <=
      (-rssfontfx.getWidth(rtcfg->getString("rss")) -
       rtcfg->getFloat("rssposxorg"))) {
    rtcfg->sub("objects")->sub("rss")->set("posx",
                                           rtcfg->getFloat("rssposxorg"));
  }
}
...
void drawrssfeed(std::string thistext) {
  ...
  glTranslatef(rtcfg->sub("objects")->sub("rss")->getFloat("posx"),
...
void loadrss() {
  if (thcfg->sub("objects")->sub("rss")->getBool("enabled")) {
    ...
    if (rssxmlcheck) {
      ...
      downloadurito(tycfg->sub("strings")->getString("rssfeed"),
                    rtcfg->getString("rssfeed"));
    }
