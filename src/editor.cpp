...
void drawsearcheditbox() {
  ...
  glTranslatef(se->getFloat("posx"), se->getFloat("posy"),
               te->getInt("posz") + 1);
  rotateme("searcheditbox");
  std::string mode;
  if (rtcfg->getBool("editmode")) {
    if (rtcfg->getBool("exporting"))
      mode = lacfg->getString("exportas");
    else
      mode = lacfg->getString("editmode");
  }
  if (rtcfg->getBool("searchmode")) {
    mode = lacfg->getString("searchmode");
  }
  ...
  if (se->getFloat("width") < settingsfontfx.getWidth(mode) + 100)
    se->set("width", settingsfontfx.getWidth(mode) + 100);
  ...
  if (rtcfg->getInt("showhelp") == helpid("themepaths") ||
      ...
      (rtcfg->getInt("showhelp") == helpid("emulatorsettings") &&
       (he->getInt("emu") == 2 || he->getInt("emu") == 5 ||
        he->getInt("emu") == 6 || he->getInt("emu") == 7))) {
...
void searchlist(std::string searchstring) {
  ...
  for (int i = Ics() + 1; i < Ime(); i++) {
    if (!foundsearch) {
      if (Iem() != 0)
        comp = rmcfg->sub("Roms")->sub(IeID() - 1)->sub(i)->getString("file");
      else
        comp = mentry(Ishowmenu() - 1, i);
...
void quitsearch() {
  ...
  if (Ishowmenu() != 0 && Ishowmenu() != 8) searchlist(thisstring);
}
...
void quitedit(std::string* toeditstring, int type) {
  ...
  switch (type) {
    ...
    case 4:  // themepaths
      thcfg->sub("paths")->setC(
          thcfg->sub("paths")->getKey(rtcfg->sub("helpsel")->getInt("paths")),
          thisstring);
      checkpaths();
      break;

    case 5:  // global paths
      tycfg->sub("paths")->setC(
          tycfg->sub("paths")->getKey(
    ...
    case 6:  // emu settings
      emcfg->sub(rtcfg->getInt("edemu"))
          ->setC(emcfg->sub(rtcfg->getInt("edemu"))
                     ->getKey(rtcfg->sub("helpsel")->getInt("emu")),
                 thisstring);
      break;
    ...
    case 9:  // global strings
      tycfg->sub("strings")->setC(
          tycfg->sub("strings")->getKey(
...
void editorinput(std::string toeditstring, int type) {
  if (!rtcfg->getBool("edsearchchanged")) {
    thisstring = toeditstring;
  }
  if (rtcfg->getBool("editmode")) {
    switch (event.key.code) {
      case 8:
