...
void setemu(int thismenu, int thisemu) {
  ...
  if (Bvidmenu()) {
    ((TConfig<int>*)thcfg->sub("objects")->sub("video")->get("videomenu"))
        ->operator-(1);
  }
}
...
void listroms() {
  FILE* romxmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("romxml").c_str(), "r");
  if (romxmlcfgcheck) {
#ifdef DEBUG
    ...
    for (int i = 0; i < emcfg->size(); i++) {
      ...
      struct dirent** sdit;
      int num_entries;
      if ((num_entries = scandir((*emuPath).c_str(), &sdit, fileselectAll,
                                 alphasort)) < 0) {
      } else {
        int k = 0;
...
void reademucfg() {
  FILE* emuxmlcfgcheck =
      fopen(rtcfg->sub("configs")->getString("emuxml").c_str(), "r");
  if (emuxmlcfgcheck) {
    loademuxmlcfg();
...
void gameid(bool up) {
  //	if (Iem()!=0)
  {
    ...
    if (up) {
      ...
      if (rom->getInt("gameid") > prcfg->sub("games")->size() - 1)
        rom->set("gameid", 0);
    } else {
      ((TConfig<int>*)rom->get("gameid"))->operator-(1);
      if (rom->getInt("gameid") < 0)
        rom->set("gameid", prcfg->sub("games")->size() - 1);
    }
    printf("gameid %i\n", rom->getInt("gameid"));
    rmcfg->saveXML(rtcfg->sub("configs")->getString("romxml"));
    forcepreviewupdate();
...
void scrapeallroms() {
  int mode = 0;
  if (compare(rtcfg->getString("scrape"), "rominfo"))
    mode = 1;
  else if (compare(rtcfg->getString("scrape"), "boxart"))
    mode = 2;
  else if (compare(rtcfg->getString("scrape"), "fanart"))
    mode = 3;
  else if (compare(rtcfg->getString("scrape"), "all"))
    mode = 4;

  if (mode == 0) {
    printf("%s\n", lacfg->getString("noscrape").c_str());
    exit(0);
  }

  else if (mode == 1) {
    printf("%s - %s\n", lacfg->getString("rominfo").c_str(),
           lacfg->getString("patient").c_str());
    ...
    printf("%s - %s\n", lacfg->getString("fanart").c_str(),
           lacfg->getString("patient").c_str());
  } else if (mode == 4) {
    printf("%s - %s\n", lacfg->getString("everything").c_str(),
           lacfg->getString("patient").c_str());
  }
}
