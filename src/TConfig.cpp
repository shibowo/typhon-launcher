...
TyphonConfig::~TyphonConfig() { delete conf; }
...
void TyphonConfig::erase(std::string key) { conf->erase(key); }
...
int* TyphonConfig::getIntP(std::string key) {
  ...
  if (strcasecmp(get(key)->typ.c_str(), "string") == 0) {
    ...
    i = (TConfig<int>*)get(key);
  return i->get();
}

int TyphonConfig::getInt(std::string key) { return *getIntP(key); }
...
float TyphonConfig::getFloat(std::string key) { return *getFloatP(key); }
...
std::string TyphonConfig::getString(int i) { return getString(getKey(i)); }

std::string* TyphonConfig::getStringP(int i) { return getStringP(getKey(i)); }

int TyphonConfig::getInt(int i) { return getInt(getKey(i)); }

int* TyphonConfig::getIntP(int i) { return getIntP(getKey(i)); }

float TyphonConfig::getFloat(int i) { return getFloat(getKey(i)); }

float* TyphonConfig::getFloatP(int i) { return getFloatP(getKey(i)); }

bool TyphonConfig::getBool(int i) { return getBool(getKey(i)); }

bool* TyphonConfig::getBoolP(int i) { return getBoolP(getKey(i)); }
  ...
  for (p = conf->begin(); p != conf->end(); p++) {
    if (strcasecmp(p->second->typ.c_str(), "map") != 0) {
      ...
      if (strcasecmp(typ.c_str(), "string") == 0)
        tagValue = getString(p->first);
      if (strcasecmp(typ.c_str(), "int") == 0)
        tagValue = getInt2String(p->first);
      if (strcasecmp(typ.c_str(), "bool") == 0)
        tagValue = BoolToString(getBool(p->first));
      if (strcasecmp(typ.c_str(), "float") == 0)
        tagValue = FloatToString(getFloat(p->first));
...
void TyphonConfig::parseXML(TyphonConfig* tc, TiXmlElement* pElem) {
  for (pElem = pElem; pElem; pElem = pElem->NextSiblingElement()) {
    ...
    } else if (strcasecmp(typ.c_str(), "string") == 0) {
      std::string stringy = "";
      if (pElem->GetText() != NULL) stringy = pElem->GetText();
      tc->set(pElem->Value(), stringy, id);
    } else if (strcasecmp(typ.c_str(), "int") == 0) {
      tc->set(pElem->Value(), atoi(pElem->GetText()), id);
    } else if (strcasecmp(typ.c_str(), "float") == 0) {
      tc->set(pElem->Value(), (float)atof(pElem->GetText()), id);
    } else if (strcasecmp(typ.c_str(), "bool") == 0) {
      const char* boolie = pElem->GetText();
      bool b = 0;
      if (!strcasecmp(boolie, "true") || !strcasecmp(boolie, "yes") ||
          !strcasecmp(boolie, "1") || !strcasecmp(boolie, "on"))
        b = 1;
...
void TyphonConfig::wipe() { conf->clear(); }

template <class T>
T TyphonConfig::addto(std::string key, T value) {
